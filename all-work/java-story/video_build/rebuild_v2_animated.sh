...
set -euo pipefail
cd /workspace
MAP_PY='all-work/java-story/video_build/distribute_v2_to_episode_prs.py'
...
# prefer output/v2 then all-work/java-story/output/v2
...
for ep in $(seq "$START" "$END"); do
  ...
  python3 -u all-work/java-story/video_build/render_v2_from_narrative.py --ep "$ep" --reuse-audio
