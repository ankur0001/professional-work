#!/usr/bin/env python3
"""Push all-work/java-story/output/v2 deliverables into each episode PR branch and retitle.

Expects v2 files already rendered under all-work/java-story/output/v2/.
Copies into each episode branch as all-work/java-story/output/v2/ (common folder name).

Usage:
  python3 all-work/java-story/video_build/distribute_v2_to_episode_prs.py --ep 1-5
  python3 all-work/java-story/video_build/distribute_v2_to_episode_prs.py --all
"""
...
ROOT = Path("/workspace")
V2 = ROOT / "all-work" / "java-story" / "output" / "v2"
NARR = ROOT / "all-work" / "java-story" / "narrative_review" / "episodes"
...
def push_episode(ep: int) -> None:
    ...
    dest = wt / "all-work" / "java-story" / "output" / "v2"
    # prefer new layout; fall back to legacy episode-PR paths
    if (wt / "java" / "output" / "v2").exists() and not (wt / "all-work" / "java-story").exists():
        dest = wt / "java" / "output" / "v2"
    elif not (wt / "all-work").exists() and not (wt / "java").exists() and (wt / "output").exists():
        dest = wt / "output" / "v2"
