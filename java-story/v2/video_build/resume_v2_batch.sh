...
while true; do
  ...
  for f in java-story/v2/output/Java_Episode_*_CAPTIONED.mp4; do
    ...
    if python3 java-story/v2/video_build/distribute_v2_to_episode_prs.py --ep "$ep" >> "$LOG" 2>&1; then
...
main() {
  ...
  if ! pgrep -f 'render_v2_from_narrative.py --ep' >/dev/null 2>&1; then
    ...
    nohup python3 -u java-story/v2/video_build/render_v2_from_narrative.py --ep "${resume}-85" > "$RENDER_LOG" 2>&1 &
  fi
  write_watcher
