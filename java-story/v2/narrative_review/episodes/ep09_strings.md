...
Arrays gave us a way to hold many values by position. That helps for scores and samples. Most applications, though, spend at least as much energy on text: a customer name, a log line, an order id, a JSON body. Text feels simple until you try to change it, compare it, or move it across a network boundary.

Start from a small action that looks harmless. You have a greeting and you want to add emphasis.
...
`a` still refers to `"hi"`. `b` refers to a new string `"hi!"`. Nothing edited the original characters in place. In Java, `String` is immutable. Concatenation does not mutate; it creates another object. That design buys safer sharing and reliable behavior when strings are used as map keys — at the cost of allocating when you keep building new text.

Why bake immutability into the type everyone uses? Because text crosses trust boundaries constantly — path names, cache keys, passwords in older APIs. If any caller could silently change the characters underneath a shared reference, security and hashing would become nightmares. Immutability is a trade: more objects sometimes, fewer alias surprises often.
...
One more boundary problem appears the moment strings leave Java characters and meet bytes — files, sockets, HTTP bodies. Encoding mistakes are silent until a name with an accent turns into garbage.
...
Always be explicit at the boundary. "The platform default charset" is not a team agreement; it is a future incident. Text and bytes are different, and the mapping between them has a name.

A loop that builds CSV with `+`, a login check that uses `==`, a file reader that assumes the default charset — each works in a demo and fails under slightly wider reality. Strings look simple because the type is everywhere. The discipline exists because the type is everywhere.
...
That pressure opens object-oriented programming.
