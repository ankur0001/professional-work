...
Output layout (common across episode PRs):
  java-story/output/v2/Java_Episode_XX_<Slug>.mp4
  java-story/output/v2/Java_Episode_XX_<Slug>_CAPTIONED.mp4
  java-story/output/v2/Java_Episode_XX.srt
  java-story/output/v2/SOURCE.md

Usage:
  python3 java-story/video_build/render_v2_from_narrative.py --ep 1
  python3 java-story/video_build/render_v2_from_narrative.py --ep 1-5 --reuse-audio
  python3 java-story/video_build/render_v2_from_narrative.py --all
"""
...
ROOT = Path("/workspace")
NARR_DIR = ROOT / "java-story" / "narrative_review" / "episodes"
BUILD = ROOT / "java-story" / "video_build"
WORK = BUILD / "v2_work"
OUT = ROOT / "java-story" / "output" / "v2"
SYS_PATH_TTS = BUILD
...
def render_one(path: Path, reuse_audio: bool = False, max_beats: int | None = None) -> dict:
    ...
    src.write_text(
        ...
        f"Narration: `java-story/narrative_review/episodes/{path.name}`\n"
        f"Renderer: `java-story/video_build/render_v2_from_narrative.py`\n"
        f"Visuals: animated scenes via `java-story/video_build/visual_engine.py`\n"
