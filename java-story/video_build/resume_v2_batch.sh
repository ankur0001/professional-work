...
for ep, br in PR_MAP.items():
    ok = False
    for path in ("output/v2", "java-story/output/v2"):
        r = subprocess.run(
...
while true; do
  shopt -s nullglob
  for f in java-story/output/v2/Java_Episode_*_CAPTIONED.mp4; do
    [ -f "$f" ] || continue
    base=$(basename "$f")
    ...
    clean_count=$(ls java-story/output/v2/Java_Episode_${ep_padded}_*.mp4 2>/dev/null | grep -v CAPTIONED | wc -l || true)
    ...
    if python3 java-story/video_build/distribute_v2_to_episode_prs.py --ep "$ep" >> "$LOG" 2>&1; then
...
main() {
  ...
  if ! pgrep -f 'render_v2_from_narrative.py --ep' >/dev/null 2>&1; then
    ...
    nohup python3 -u java-story/video_build/render_v2_from_narrative.py --ep "${resume}-85" > "$RENDER_LOG" 2>&1 &
  fi
  write_watcher
