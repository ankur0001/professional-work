...
set -euo pipefail
cd /workspace
MAP_PY='java-story/video_build/distribute_v2_to_episode_prs.py'
OUT=java-story/output/v2
mkdir -p "$OUT"
...
# prefer output/v2 then java-story/output/v2
...
for ep in $(seq "$START" "$END"); do
  ...
  python3 -u java-story/video_build/render_v2_from_narrative.py --ep "$ep" --reuse-audio
  python3 -u java-story/video_build/distribute_v2_to_episode_prs.py --ep "$ep"
