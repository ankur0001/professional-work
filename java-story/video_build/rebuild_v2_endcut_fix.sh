...
for ep in $(seq "$START" "$END"); do
  ...
  aud="java-story/video_build/v2_work/ep$(printf '%02d' "$ep")/audio"
  if [ -d "$aud" ]; then
    beats=( "$aud"/b*.wav )
    nbeats=${#beats[@]}
    if [ "$nbeats" -eq 0 ]; then
  ...
  python3 -u java-story/video_build/render_v2_from_narrative.py --ep "$ep"
