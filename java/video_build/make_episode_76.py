#!/usr/bin/env python3
"""Episode 76 — Spring Security. Narration + visuals authored together."""
...
AUDIO, FRAMES, CLIPS = ROOT / "audio_ep76", ROOT / "frames_ep76", ROOT / "clips_ep76"
VOICE = os.environ.get("KOKORO_VOICE", "am_michael")
SPEED = float(os.environ.get("KOKORO_SPEED", "0.97"))

SCENES = [
    ("hook", "hook", [
        'Episode Seventy-Five covered Spring Data, transactions, and N-plus-one pitfalls.',
        'Persistence without security is an open door — every API needs an access story.',
        'Spring Security is the standard filter chain for authentication and authorization.',
        'Modern apps mix sessions, JWTs, OAuth two, and method-level rules.',
        'Interviews expect you to separate who you are from what you may do.',
        'Today — authn versus authz, the filter chain, securing REST, and common traps.',
    ]),
    ("title", "title", [
        'Episode Seventy-Six.',
        'Spring Security.',
    ...
    ("authn_authz", "authn_authz", [
        ...
        'Credentials become a SecurityContext with an Authentication principal.',
        'Roles and authorities express permissions — ROLE_USER is a convention, not magic.',
        'Fail closed — deny by default, grant explicitly.',
        'Never conflate logged-in with allowed — that is the classic auth bug.',
        'Say authn and authz separately in interviews — interviewers listen for the split.',
    ]),
    ("filter_chain", "filter_chain", [
        'Spring Security is a filter chain in front of your servlet stack.',
        'SecurityFilterChain beans declare which paths need auth and which are public.',
        'UsernamePasswordAuthenticationFilter, BearerTokenAuthenticationFilter — specialized links.',
        'Once authenticated, AuthorizationFilter enforces request matchers and rules.',
        'Order matters — mis-ordered filters create confusing allow or deny behavior.',
        'Debug with spring-security DEBUG logs when a path is unexpectedly open.',
    ]),
    ("rest_apis", "rest_apis", [
        'Securing REST APIs in practice.',
        'Stateless JWT or opaque tokens for services — sessions for browser apps.',
        'CSRF protection matters for cookie sessions — often disabled for pure bearer APIs.',
        'CORS is not security — it only relaxes browser same-origin rules.',
        'Use HTTPS everywhere — tokens on the wire are credentials.',
        'Resource servers validate tokens — authorization servers issue them.',
    ]),
    ("method_security", "method_security", [
        'Method security complements URL rules.',
        'EnableMethodSecurity unlocks PreAuthorize and PostAuthorize.',
        'Express domain rules — hasRole, hasAuthority, or custom PermissionEvaluator.',
        'URL rules catch coarse paths — method rules protect service operations.',
        'SpEL in annotations is powerful — keep expressions readable and tested.',
        'Defense in depth — never rely on the UI alone to hide forbidden actions.',
    ]),
    ("practices", "practices", [
        'Security practices that survive production.',
        'Least privilege — grant the smallest authority that works.',
        'Rotate secrets and signing keys — store them outside the repo.',
        'Log auth failures without leaking whether a username exists when policy forbids it.',
        'Test both positive and negative authorization paths in CI.',
        'Threat-model new endpoints — every PostMapping is an attack surface.',
    ]),
    ("mistakes", "mistakes", [
        'Three common mistakes.',
        'One — permitAll on a broad ant pattern that swallows admin routes.',
        'Two — trusting client-sent roles inside an unsigned JWT payload.',
        'Three — disabling CSRF for cookie apps without understanding the risk.',
        'Also — putting authorization only in the frontend — backends must enforce.',
        'Security is a server responsibility — clients are untrusted.',
    ]),
    ("interview", "interview", [
        'Interview question — how do you secure a Spring Boot REST API?',
        'Declare a SecurityFilterChain — authenticate requests, authorize paths.',
        'Prefer bearer tokens for stateless services — validate signatures and expiry.',
        'Map authorities to endpoints and critical service methods.',
        'Keep secrets external — HTTPS in transit, least privilege in design.',
        'Prove it with tests that assert four-oh-one and four-oh-three outcomes.',
    ]),
    ("teaser", "teaser", [
        'Security holds the line — next we prove the system.',
        'Episode Seventy-Seven — Spring Testing.',
        'Unit, slice, and full-context tests that keep Spring apps honest.',
        'See you there.',
    ]),
]
...
def render_hook(progress, t):
    ...
    labs = [('authn', 'ORANGE'), ('authz', 'BLUE'), ('filters', 'GREEN')]
...
def render_title(progress, t):
    ...
    for txt, fnt, y, col in [
        ('EPISODE 76', font(FONT_BOLD, 28), H // 2 - 140, mix(BG, MUTED, a)),
        ('Spring Security', font(FONT_SERIF, 46), H // 2 - 30, mix(BG, WHITE, a)),
        ('authn · authz · filter chain · REST', font(FONT_REG, 26), H // 2 + 70, mix(BG, MUTED, a)),
    ]:
        bbox = d.textbbox((0, 0), txt, font=fnt); d.text(((W - (bbox[2] - bbox[0])) // 2, y), txt, font=fnt, fill=col)
    return img.convert("RGB")

def render_authn_authz(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Authn vs Authz', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('authentication', 'who you are', 'ORANGE'), ('authorization', 'what you may do', 'BLUE'), ('fail closed', 'deny by default', 'GREEN')]
...
def render_filter_chain(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Filter Chain', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('SecurityFilterChain', 'path rules + auth entry', 'ORANGE'), ('specialized filters', 'form / bearer / basic', 'BLUE'), ('AuthorizationFilter', 'enforce matchers', 'GREEN')]
    for i, (k, v, col) in enumerate(items):
        a = ease_out_cubic(clamp((progress - i * 0.18) / 0.3))
        if a <= 0: continue
        y = 220 + i * 200
...
def render_rest_apis(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Securing REST', font=font(FONT_SERIF, 40), fill=WHITE)
    ...
    lines = ['stateless JWT / opaque tokens', 'CSRF for cookie sessions', 'CORS ≠ security', 'HTTPS for tokens in transit', 'resource vs authorization server']
    for i, line in enumerate(lines):
        aa = ease_out_cubic(clamp((progress - i * 0.08) / 0.22))
        d.text((260, 280 + i * 100), line, font=font(FONT_MONO, 26), fill=mix(BG, WHITE, aa))
    return img.convert("RGB")

def render_method_security(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Method Security', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('@PreAuthorize', 'domain rules on methods', 'ORANGE'), ('URL + method', 'defense in depth', 'BLUE'), ('test expressions', 'keep SpEL readable', 'GREEN')]
    for i, (k, v, col) in enumerate(items):
        a = ease_out_cubic(clamp((progress - i * 0.18) / 0.3))
        if a <= 0: continue
        y = 220 + i * 200
...
def render_practices(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Practices', font=font(FONT_SERIF, 44), fill=WHITE)
    steps = [('least privilege', 'ORANGE'), ('rotate secrets', 'BLUE'), ('neg. path tests', 'GREEN'), ('threat-model APIs', 'RED')]
...
def render_mistakes(progress, t):
    ...
    items = [('01', 'broad permitAll', 'scope public paths tightly'), ('02', 'trust unsigned roles', 'validate token signatures'), ('03', 'auth only in UI', 'enforce on the server')]
...
def render_interview(progress, t):
    ...
    q = 'How do you secure a Boot REST API?'
    bbox = d.textbbox((0, 0), q, font=font(FONT_BOLD, 30)); d.text(((W - (bbox[2] - bbox[0])) // 2, 190), q, font=font(FONT_BOLD, 30), fill=WHITE)
    answers = [('filter chain', 'authenticate + authorize', 'ORANGE'), ('tokens', 'validate sig + expiry', 'BLUE'), ('prove it', '401/403 tests in CI', 'GREEN')]
...
def render_teaser(progress, t):
    ...
    title = 'Spring Testing'; bbox = d.textbbox((0, 0), title, font=font(FONT_SERIF, 42))
    d.text(((W - (bbox[2] - bbox[0])) // 2, H // 2 - 40), title, font=font(FONT_SERIF, 42), fill=WHITE)
    sub = 'unit · slice · full context'; bbox = d.textbbox((0, 0), sub, font=font(FONT_REG, 28))
    d.text(((W - (bbox[2] - bbox[0])) // 2, H // 2 + 60), sub, font=font(FONT_REG, 28), fill=BLUE)
    d.text((W // 2 - 90, H // 2 + 150), 'Episode 77', font=font(FONT_BOLD, 30), fill=ORANGE)
    return img.convert("RGB")

RENDERERS = {
    "hook": render_hook,
    "title": render_title,
    "authn_authz": render_authn_authz,
    "filter_chain": render_filter_chain,
    "rest_apis": render_rest_apis,
    "method_security": render_method_security,
...
def main():
    ...
    (ROOT / "ep76_durations.json").write_text(json.dumps(durations, indent=2))
    outs = [render_scene_clip(sid, r, durations[sid]) for sid, r, _ in SCENES]
    lst = ROOT / "concat_ep76.txt"
    with open(lst, "w") as f:
        for p in outs: f.write(f"file '{p}'\n")
    narrated = OUTPUT / "java_ep76_narrated.mp4"
    ...
    if abs(pace - 1.0) > 0.015:
        ...
        paced = OUTPUT / "java_ep76_paced.mp4"
        ...
        base = paced
    final = OUTPUT / "Java_Episode_76_Spring_Security.mp4"
    ...
    srt = OUTPUT / "Java_Episode_76.srt"; write_srt(durations, srt); shutil.copy2(srt, ARTIFACTS / srt.name)
    burned = OUTPUT / "Java_Episode_76_Spring_Security_CAPTIONED.mp4"
    ...
    vdir = ARTIFACTS / "ep76_verify"; vdir.mkdir(exist_ok=True)
    for ts, name in [('00:00:12', '01_hook'), ('00:00:50', '02_authn'), ('00:01:40', '03_filters'), ('00:02:30', '04_rest'), ('00:03:20', '05_interview')]:
        subprocess.run(["ffmpeg", "-y", "-ss", ts, "-i", str(final), "-frames:v", "1", str(vdir / f"{name}.jpg")], capture_output=True)
    final_dur = probe(final)
    print(f"DONE Episode 76: {final_dur/60:.2f} min")
