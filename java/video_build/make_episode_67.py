#!/usr/bin/env python3
"""Episode 67 — Design Patterns Intro. Narration + visuals authored together."""
...
AUDIO, FRAMES, CLIPS = ROOT / "audio_ep67", ROOT / "frames_ep67", ROOT / "clips_ep67"
VOICE = os.environ.get("KOKORO_VOICE", "am_michael")
SPEED = float(os.environ.get("KOKORO_SPEED", "0.97"))

SCENES = [
    ("hook", "hook", [
        'Episode Sixty-Six wrapped JVM interview answers — heap, GC, JIT, and measurement.',
        'Architecture interviews shift next — from how the JVM runs code to how you structure code.',
        'Design patterns are reusable solutions to recurring software design problems.',
        'They are not libraries — they are named ideas teams use to communicate intent.',
        'Junior engineers memorize names — seniors know when a pattern helps and when it hurts.',
        'Today — what patterns are, the three GoF categories, and how Java already uses them.',
    ]),
    ("title", "title", [
        'Episode Sixty-Seven.',
    ...
    ("what_patterns", "what_patterns", [
        'A design pattern names a proven structure for a common problem.',
        'The Gang of Four book cataloged twenty-three classic object-oriented patterns.',
        'Each pattern has a problem, a structure, consequences, and known uses.',
        'Patterns create a shared vocabulary — say Adapter and teammates know the shape.',
        'They are tools for clarity — not badges to sprinkle on every class.',
        'Prefer the simplest design that communicates intent — patterns when complexity earns them.',
    ]),
    ("three_categories", "three_categories", [
        'Patterns group into three categories by intent.',
        ...
        'Behavioral — organize communication and algorithms — Strategy, Observer, Command.',
        'Episodes Sixty-Eight through Seventy walk each category with Java examples.',
        'Category first — then pick the pattern that matches the force you are balancing.',
    ]),
    ("java_already", "java_already", [
        "Java's standard library is full of patterns you already use.",
        'Iterator — for-each loops walk collections without exposing structure.',
        'Observer — listeners and reactive streams notify interested parties.',
        'Decorator — java.io streams wrap streams — BufferedInputStream over FileInputStream.',
        'Factory — Calendar.getInstance and Paths.get hide concrete construction.',
        'Recognizing patterns in the JDK trains your eye for application design.',
    ]),
    ("when_to_use", "when_to_use", [
        'Use a pattern when the problem matches — not when you want a fancy name.',
        'Duplicated construction logic — consider Factory or Builder.',
        'Need to swap algorithms at runtime — Strategy fits cleanly.',
        'Legacy API mismatch — Adapter bridges without rewriting callers.',
        'If a pattern adds classes without reducing coupling — skip it.',
        'Readability for your team beats purity from a textbook.',
    ]),
    ("anti_patterns", "anti_patterns", [
        'Pattern abuse is a real failure mode.',
        'Singleton everywhere — hidden global state that breaks tests.',
        'Abstract factory for two concrete types — ceremony without payoff.',
        'Observer graphs so tangled that change ripples unpredictably.',
        'Also — renaming a simple method call a Strategy without a family of algorithms.',
        'Patterns serve design — design does not serve pattern checklists.',
    ]),
    ("mistakes", "mistakes", [
        'Three common mistakes.',
        'One — memorizing twenty-three names without a problem they solve.',
        'Two — forcing patterns into code that is already clear.',
        'Three — confusing design patterns with architectural styles like microservices.',
        'Also — skipping trade-offs — every pattern adds indirection cost.',
        'Name the force, then the pattern — never the reverse.',
    ]),
    ("interview", "interview", [
        'Interview question — what is a design pattern and why use one?',
        'A named, reusable solution to a recurring design problem.',
        'It improves communication — teams share intent with one word.',
        'It packages trade-offs — you adopt known consequences deliberately.',
        "Java's JDK uses patterns heavily — Iterator, Decorator, Factory.",
        'Use them when they reduce coupling or clarify variation — not for prestige.',
    ]),
    ("teaser", "teaser", [
        'Next we go deep on object creation.',
        'Episode Sixty-Eight — Creational Patterns.',
...
def render_hook(progress, t):
    ...
    labs = [('Creational', 'ORANGE'), ('Structural', 'BLUE'), ('Behavioral', 'GREEN')]
...
def render_title(progress, t):
    ...
    for txt, fnt, y, col in [
        ('EPISODE 67', font(FONT_BOLD, 28), H // 2 - 140, mix(BG, MUTED, a)),
        ('Design Patterns Intro', font(FONT_SERIF, 48), H // 2 - 30, mix(BG, WHITE, a)),
        ('GoF vocabulary · JDK already uses them', font(FONT_REG, 26), H // 2 + 70, mix(BG, MUTED, a)),
    ]:
        bbox = d.textbbox((0, 0), txt, font=fnt); d.text(((W - (bbox[2] - bbox[0])) // 2, y), txt, font=fnt, fill=col)
    return img.convert("RGB")

def render_what_patterns(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'What Is a Pattern?', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('named solution', 'recurring design problem', 'ORANGE'), ('shared vocabulary', 'Adapter means one shape', 'BLUE'), ('trade-offs included', 'adopt consequences on purpose', 'GREEN')]
...
def render_three_categories(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Three Categories', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('Creational', 'how objects are born', 'ORANGE'), ('Structural', 'how objects compose', 'BLUE'), ('Behavioral', 'how objects collaborate', 'GREEN')]
    for i, (k, v, col) in enumerate(items):
        a = ease_out_cubic(clamp((progress - i * 0.18) / 0.3))
        if a <= 0: continue
        y = 220 + i * 200
...
def render_java_already(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'In the JDK Already', font=font(FONT_SERIF, 44), fill=WHITE)
    ...
    lines = ['Iterator — for-each over collections', 'Decorator — nested java.io streams', 'Observer — listeners / callbacks', 'Factory — Calendar.getInstance()', 'Factory — Paths.get(...)']
...
def render_when_to_use(progress, t):
    ...
    steps = [('match force', 'ORANGE'), ('name pattern', 'BLUE'), ('weigh cost', 'GREEN'), ('keep simple', 'RED')]
...
def render_anti_patterns(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Pattern Abuse', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('Singleton spam', 'hidden globals, hard tests', 'ORANGE'), ('factory theater', 'ceremony for two types', 'BLUE'), ('tangled observers', 'unpredictable ripples', 'GREEN')]
    for i, (k, v, col) in enumerate(items):
        a = ease_out_cubic(clamp((progress - i * 0.18) / 0.3))
        if a <= 0: continue
        y = 220 + i * 200
...
def render_mistakes(progress, t):
    ...
    items = [('01', 'memorize 23 names', 'start from the problem'), ('02', 'force a pattern', 'keep clear code clear'), ('03', 'patterns = architecture', 'different levels')]
...
def render_interview(progress, t):
    ...
    answers = [('named solution', 'recurring design problem', 'ORANGE'), ('shared language', 'one word for intent', 'BLUE'), ('use when earned', 'cut coupling, clarify variation', 'GREEN')]
...
def render_teaser(progress, t):
    ...
    title = 'Creational Patterns'; bbox = d.textbbox((0, 0), title, font=font(FONT_SERIF, 42))
    d.text(((W - (bbox[2] - bbox[0])) // 2, H // 2 - 40), title, font=font(FONT_SERIF, 42), fill=WHITE)
    sub = 'Singleton · Factory · Builder'; bbox = d.textbbox((0, 0), sub, font=font(FONT_REG, 28))
    d.text(((W - (bbox[2] - bbox[0])) // 2, H // 2 + 60), sub, font=font(FONT_REG, 28), fill=BLUE)
    d.text((W // 2 - 90, H // 2 + 150), 'Episode 68', font=font(FONT_BOLD, 30), fill=ORANGE)
    return img.convert("RGB")

RENDERERS = {
    "hook": render_hook,
    "title": render_title,
    "what_patterns": render_what_patterns,
    "three_categories": render_three_categories,
    "java_already": render_java_already,
    "when_to_use": render_when_to_use,
...
def main():
    ...
    (ROOT / "ep67_durations.json").write_text(json.dumps(durations, indent=2))
    outs = [render_scene_clip(sid, r, durations[sid]) for sid, r, _ in SCENES]
    lst = ROOT / "concat_ep67.txt"
    with open(lst, "w") as f:
        for p in outs: f.write(f"file '{p}'\n")
    narrated = OUTPUT / "java_ep67_narrated.mp4"
    ...
    if abs(pace - 1.0) > 0.015:
        ...
        paced = OUTPUT / "java_ep67_paced.mp4"
        ...
        base = paced
    final = OUTPUT / "Java_Episode_67_Design_Patterns_Intro.mp4"
    ...
    srt = OUTPUT / "Java_Episode_67.srt"; write_srt(durations, srt); shutil.copy2(srt, ARTIFACTS / srt.name)
    burned = OUTPUT / "Java_Episode_67_Design_Patterns_Intro_CAPTIONED.mp4"
    ...
    vdir = ARTIFACTS / "ep67_verify"; vdir.mkdir(exist_ok=True)
    for ts, name in [('00:00:12', '01_hook'), ('00:00:50', '02_what'), ('00:01:40', '03_categories'), ('00:02:30', '04_jdk'), ('00:03:20', '05_interview')]:
        subprocess.run(["ffmpeg", "-y", "-ss", ts, "-i", str(final), "-frames:v", "1", str(vdir / f"{name}.jpg")], capture_output=True)
    final_dur = probe(final)
    print(f"DONE Episode 67: {final_dur/60:.2f} min")
