#!/usr/bin/env python3
"""Episode 75 — Spring Data and Persistence. Narration + visuals authored together."""
...
AUDIO, FRAMES, CLIPS = ROOT / "audio_ep75", ROOT / "frames_ep75", ROOT / "clips_ep75"
VOICE = os.environ.get("KOKORO_VOICE", "am_michael")
SPEED = float(os.environ.get("KOKORO_SPEED", "0.97"))

SCENES = [
    ("hook", "hook", [
        'Episode Seventy-Four covered Spring MVC and REST boundaries.',
        'Most services persist state — Spring Data makes repositories a first-class idea.',
        'JPA and Hibernate remain the common relational stack under Spring Data JPA.',
        'Transactions define consistency boundaries — get them wrong and data lies.',
        'Performance interviews often land on lazy loading and N-plus-one queries.',
        'Today — repositories, entities, transactions, and persistence pitfalls.',
    ]),
    ("title", "title", [
    ...
    ("repositories", "repositories", [
        'Spring Data repositories declare persistence intent as interfaces.',
        'JpaRepository gives CRUD, paging, and sorting with almost no boilerplate.',
        'Query methods derive SQL from method names — findByEmail, existsById.',
        'Query annotation or specs handle complex predicates deliberately.',
        'Keep repositories focused on persistence — not business workflows.',
        'Custom fragments extend repositories when derived queries run out.',
    ]),
    ("entities", "entities", [
        'Entities map tables to objects — carefully.',
        'Id and GeneratedValue define identity — equals should respect it thoughtfully.',
        'Relationships — OneToMany, ManyToOne — model cardinality and ownership.',
        'FetchType lazy is the default for many associations — for good reason.',
        'DTOs or projections keep API models separate from persistence models.',
        'Schema migrations belong in Flyway or Liquibase — not hope and hibernate-ddl.',
    ]),
    ("transactions", "transactions", [
        'Transactional marks a consistency boundary.',
        'On a service facade — one use case, one transaction, by default.',
        'Propagation and isolation settings change when nested calls join or suspend.',
        'Read-only transactions can help readers — still measure before assuming.',
        'Checked exceptions do not roll back by default — RuntimeException does.',
        'Keep transactions short — no remote calls while holding DB locks.',
    ]),
    ("nplusone", "nplusone", [
        'The N-plus-one problem — classic interview and production trap.',
        'One query loads parents — N more queries load each child collection.',
        'Symptoms — fine in tests with two rows, painful with thousands.',
        'Fixes — join fetch, entity graphs, or dedicated query DTOs.',
        'Open Session in View can hide the issue until you disable it.',
        'Log SQL in staging — count queries for critical endpoints.',
    ]),
    ("practices", "practices", [
        'Practical persistence practices.',
        'Index for the queries you run — not for every column.',
        'Paginate at the database — not in memory after findAll.',
        'Optimistic locking with Version prevents silent overwrites.',
        'Separate read models when query shapes diverge from write models.',
        'Measure with realistic data volumes — microbenchmarks lie.',
    ]),
    ("mistakes", "mistakes", [
        'Three common mistakes.',
        'One — calling findAll and filtering in Java — database should filter.',
        'Two — lazy loads in JSON serialization — accidental query storms.',
        'Three — huge transactional methods that call other services.',
        'Also — exposing entities over REST — persistence leaks into clients.',
        'Persistence is a boundary — protect it with clear services and DTOs.',
    ]),
    ("interview", "interview", [
        'Interview question — how do you avoid N-plus-one with Spring Data JPA?',
        'Detect it — enable SQL logging and watch query counts under load.',
        'Fetch joins or entity graphs load required associations deliberately.',
        'DTO projections query exactly the columns the use case needs.',
        'Avoid Open Session in View masking lazy loads during rendering.',
        'Fix the query shape — do not just raise the connection pool size.',
    ]),
    ("teaser", "teaser", [
        'Data is safe — next we lock the doors.',
        'Episode Seventy-Six — Spring Security.',
        'Authentication, authorization, filters, and securing REST APIs.',
        'See you there.',
    ]),
]
...
def render_title(progress, t):
    ...
    for txt, fnt, y, col in [
        ('EPISODE 75', font(FONT_BOLD, 28), H // 2 - 140, mix(BG, MUTED, a)),
        ('Spring Data & Persistence', font(FONT_SERIF, 48), H // 2 - 30, mix(BG, WHITE, a)),
        ('JPA · repositories · transactions · N+1', font(FONT_REG, 26), H // 2 + 70, mix(BG, MUTED, a)),
    ]:
        bbox = d.textbbox((0, 0), txt, font=fnt); d.text(((W - (bbox[2] - bbox[0])) // 2, y), txt, font=fnt, fill=col)
    return img.convert("RGB")

def render_repositories(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Repositories', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('JpaRepository', 'CRUD + paging built in', 'ORANGE'), ('derived queries', 'findByEmail style names', 'BLUE'), ('@Query / specs', 'complex predicates', 'GREEN')]
...
def render_entities(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Entities', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('identity', '@Id + generation', 'ORANGE'), ('relations', 'cardinality + ownership', 'BLUE'), ('DTO boundary', "don't leak entities", 'GREEN')]
    for i, (k, v, col) in enumerate(items):
        a = ease_out_cubic(clamp((progress - i * 0.18) / 0.3))
        if a <= 0: continue
        y = 220 + i * 200
...
def render_transactions(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Transactions', font=font(FONT_SERIF, 40), fill=WHITE)
    ...
    lines = ['@Transactional on services', 'short boundaries, no remote calls', 'RuntimeException → rollback', 'checked exceptions need setup', 'propagation defines join/suspend']
    for i, line in enumerate(lines):
        aa = ease_out_cubic(clamp((progress - i * 0.08) / 0.22))
        d.text((260, 280 + i * 100), line, font=font(FONT_MONO, 26), fill=mix(BG, WHITE, aa))
    return img.convert("RGB")

def render_nplusone(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'N+1 Queries', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('1 + N selects', 'parents then each child', 'ORANGE'), ('join fetch / graph', 'load what you need', 'BLUE'), ('log SQL', 'count queries in staging', 'GREEN')]
    for i, (k, v, col) in enumerate(items):
        a = ease_out_cubic(clamp((progress - i * 0.18) / 0.3))
        if a <= 0: continue
        y = 220 + i * 200
...
def render_practices(progress, t):
    ...
    steps = [('index queries', 'ORANGE'), ('paginate in DB', 'BLUE'), ('@Version locks', 'GREEN'), ('measure volume', 'RED')]
...
def render_mistakes(progress, t):
    ...
    items = [('01', 'findAll then filter', 'push predicates to SQL'), ('02', 'lazy in serialization', 'fetch or project first'), ('03', 'fat transactions', 'keep DB work short')]
...
def render_interview(progress, t):
    ...
    q = 'How do you avoid N+1 in Spring Data?'
    bbox = d.textbbox((0, 0), q, font=font(FONT_BOLD, 32)); d.text(((W - (bbox[2] - bbox[0])) // 2, 190), q, font=font(FONT_BOLD, 32), fill=WHITE)
    answers = [('detect', 'log SQL + count', 'ORANGE'), ('fetch shape', 'join / entity graph', 'BLUE'), ('DTO project', 'select only needed cols', 'GREEN')]
...
def render_teaser(progress, t):
    ...
    title = 'Spring Security'; bbox = d.textbbox((0, 0), title, font=font(FONT_SERIF, 42))
    d.text(((W - (bbox[2] - bbox[0])) // 2, H // 2 - 40), title, font=font(FONT_SERIF, 42), fill=WHITE)
    sub = 'authn · authz · securing REST'; bbox = d.textbbox((0, 0), sub, font=font(FONT_REG, 28))
    d.text(((W - (bbox[2] - bbox[0])) // 2, H // 2 + 60), sub, font=font(FONT_REG, 28), fill=BLUE)
    d.text((W // 2 - 90, H // 2 + 150), 'Episode 76', font=font(FONT_BOLD, 30), fill=ORANGE)
    return img.convert("RGB")
...
def main():
    ...
    (ROOT / "ep75_durations.json").write_text(json.dumps(durations, indent=2))
    outs = [render_scene_clip(sid, r, durations[sid]) for sid, r, _ in SCENES]
    lst = ROOT / "concat_ep75.txt"
    with open(lst, "w") as f:
        for p in outs: f.write(f"file '{p}'\n")
    narrated = OUTPUT / "java_ep75_narrated.mp4"
    ...
    if abs(pace - 1.0) > 0.015:
        ...
        paced = OUTPUT / "java_ep75_paced.mp4"
        ...
        base = paced
    final = OUTPUT / "Java_Episode_75_Spring_Data_Persistence.mp4"
    ...
    srt = OUTPUT / "Java_Episode_75.srt"; write_srt(durations, srt); shutil.copy2(srt, ARTIFACTS / srt.name)
    burned = OUTPUT / "Java_Episode_75_Spring_Data_Persistence_CAPTIONED.mp4"
    ...
    vdir = ARTIFACTS / "ep75_verify"; vdir.mkdir(exist_ok=True)
    for ts, name in [('00:00:12', '01_hook'), ('00:00:50', '02_repos'), ('00:01:40', '03_tx'), ('00:02:30', '04_nplusone'), ('00:03:20', '05_interview')]:
        subprocess.run(["ffmpeg", "-y", "-ss", ts, "-i", str(final), "-frames:v", "1", str(vdir / f"{name}.jpg")], capture_output=True)
    final_dur = probe(final)
    print(f"DONE Episode 75: {final_dur/60:.2f} min")
