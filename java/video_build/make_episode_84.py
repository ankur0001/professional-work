#!/usr/bin/env python3
"""Episode 84 — Performance Playbook (Season 2). Narration + visuals authored together."""
...
AUDIO, FRAMES, CLIPS = ROOT / "audio_ep84", ROOT / "frames_ep84", ROOT / "clips_ep84"
VOICE = os.environ.get("KOKORO_VOICE", "am_michael")
SPEED = float(os.environ.get("KOKORO_SPEED", "0.97"))

SCENES = [
    ("hook", "hook", [
        'Episode Eighty-Three covered event-driven flows and the transactional outbox.',
        'Performance work without a method becomes random tuning theater.',
        'Users feel latency percentiles — not average CPU on a quiet afternoon.',
        'Java gives you JFR, async profilers, GC logs, and Micrometer — use them.',
        'Fix the hottest path first — then re-measure.',
        'Today — a performance playbook from symptom to verified fix.',
    ]),
    ("title", "title", [
        'Episode Eighty-Four.',
    ...
    ("measure", "measure", [
        'Start with measurement — never with a hunch.',
        'Define the SLO — p95 latency, error rate, throughput under load.',
        'Reproduce with realistic data volumes and concurrency.',
        'Capture RED metrics — rate, errors, duration — per endpoint.',
        'Add traces for the slow requests — see where time goes.',
        'Without a baseline, you cannot claim an improvement.',
    ]),
    ("find", "find", [
        'Find the bottleneck before changing code.',
        'CPU-bound — profile methods — hot loops, JSON, crypto, regex.',
        'IO-bound — watch DB time, remote calls, thread pool waits.',
        'Allocation pressure — GC logs and allocation profilers tell the story.',
        'Lock contention — thread dumps and JFR synchronization events.',
        'One primary bottleneck at a time — resist shotgun refactors.',
    ]),
    ("fix_app", "fix_app", [
        'Application-level fixes that often win.',
        'Cut N-plus-one queries — fetch joins or DTO projections.',
        'Cache correct hot reads — with invalidation from Episode Eighty-One.',
        'Shrink payloads — less JSON, fewer columns, pagination.',
        'Reuse connections and HTTP clients — connection setup is not free.',
        'Move heavy work off the request thread — async or queues when product allows.',
    ]),
    ("fix_runtime", "fix_runtime", [
        'Runtime and platform levers.',
        'Size heaps and pick collectors from GC evidence — Episode Sixty-Two.',
        'Tune thread and pool sizes to measured concurrency — not folklore.',
        'Warm up JIT before load tests that claim peak performance.',
        'Watch container CPU throttling — limits can fake a slow app.',
        'Native images and AOT help startup — steady-state still needs profiling.',
    ]),
    ("verify", "verify", [
        'Verify like a scientist.',
        'One change, then re-run the same load scenario.',
        'Compare p95 and p99 — not only averages.',
        'Watch error rate — a faster endpoint that five-hundreds is not a win.',
        'Keep a short performance note — what changed, what improved.',
        'Automate a smoke performance check in CI for critical paths when feasible.',
    ]),
    ("mistakes", "mistakes", [
        'Three common mistakes.',
        'One — optimizing before measuring — classic premature work.',
        'Two — load testing with tiny datasets — missing real index and GC behavior.',
        'Three — celebrating mean latency while p99 burns.',
        'Also — changing five knobs at once — no attribution.',
        'Method beats folklore — every time.',
    ]),
    ("interview", "interview", [
        "Interview question — an endpoint's p99 doubled after a release — what do you do?",
        'Confirm blast radius — which endpoint, since when, which region.',
        'Compare metrics and traces before versus after the deploy.',
        'Profile or pull JFR — classify CPU, IO, GC, or lock wait.',
        'Fix the top offender — re-measure under the same load.',
        'Ship with a rollback plan — performance incidents need escape hatches.',
    ]),
    ("teaser", "teaser", [
        'Speed is one production virtue — readiness is the checklist.',
        'Episode Eighty-Five — Production Readiness Capstone.',
        'The Season Two finale — ship checklist and staff-level wrap.',
        'See you there.',
    ]),
]
...
def render_hook(progress, t):
    ...
    labs = [('baseline', 'ORANGE'), ('profile', 'BLUE'), ('verify', 'GREEN')]
...
def render_title(progress, t):
    ...
    for txt, fnt, y, col in [
        ('EPISODE 84 · S2', font(FONT_BOLD, 28), H // 2 - 140, mix(BG, MUTED, a)),
        ('Performance Playbook', font(FONT_SERIF, 46), H // 2 - 30, mix(BG, WHITE, a)),
        ('SLO · bottleneck · hottest path', font(FONT_REG, 26), H // 2 + 70, mix(BG, MUTED, a)),
    ]:
        bbox = d.textbbox((0, 0), txt, font=fnt); d.text(((W - (bbox[2] - bbox[0])) // 2, y), txt, font=fnt, fill=col)
    return img.convert("RGB")

def render_measure(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Measure First', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('define SLO', 'p95 · errors · throughput', 'ORANGE'), ('realistic load', 'data volume + concurrency', 'BLUE'), ('traces on slow', 'where time goes', 'GREEN')]
...
def render_find(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Find Bottleneck', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('CPU vs IO', 'profile or wait time', 'ORANGE'), ('GC / alloc', 'allocation pressure', 'BLUE'), ('locks', 'contention in JFR', 'GREEN')]
    for i, (k, v, col) in enumerate(items):
        a = ease_out_cubic(clamp((progress - i * 0.18) / 0.3))
        if a <= 0: continue
        y = 220 + i * 200
...
def render_fix_app(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'App Fixes', font=font(FONT_SERIF, 40), fill=WHITE)
    ...
    lines = ['kill N+1 queries', 'cache correct hot reads', 'shrink payloads + paginate', 'reuse HTTP/DB pools', 'offload heavy work when OK']
    for i, line in enumerate(lines):
        aa = ease_out_cubic(clamp((progress - i * 0.08) / 0.22))
        d.text((260, 280 + i * 100), line, font=font(FONT_MONO, 26), fill=mix(BG, WHITE, aa))
    return img.convert("RGB")

def render_fix_runtime(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Runtime Levers', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('GC + heap', 'evidence from logs', 'ORANGE'), ('pools', 'size to concurrency', 'BLUE'), ('warm JIT', 'before peak claims', 'GREEN')]
    for i, (k, v, col) in enumerate(items):
        a = ease_out_cubic(clamp((progress - i * 0.18) / 0.3))
        if a <= 0: continue
        y = 220 + i * 200
...
def render_verify(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Verify', font=font(FONT_SERIF, 44), fill=WHITE)
    steps = [('one change', 'ORANGE'), ('same load', 'BLUE'), ('p95/p99', 'GREEN'), ('write it down', 'RED')]
...
def render_mistakes(progress, t):
    ...
    items = [('01', 'optimize before measure', 'baseline first'), ('02', 'tiny load data', 'realistic volumes'), ('03', 'mean-only wins', 'watch p99 + errors')]
...
def render_interview(progress, t):
    ...
    q = 'p99 doubled after release — steps?'
    bbox = d.textbbox((0, 0), q, font=font(FONT_BOLD, 30)); d.text(((W - (bbox[2] - bbox[0])) // 2, 190), q, font=font(FONT_BOLD, 30), fill=WHITE)
    answers = [('confirm scope', 'endpoint · when · where', 'ORANGE'), ('diff traces', 'classify CPU/IO/GC', 'BLUE'), ('fix + remeasure', 'rollback ready', 'GREEN')]
...
def render_teaser(progress, t):
    ...
    title = 'Production Readiness Capstone'; bbox = d.textbbox((0, 0), title, font=font(FONT_SERIF, 42))
    d.text(((W - (bbox[2] - bbox[0])) // 2, H // 2 - 40), title, font=font(FONT_SERIF, 42), fill=WHITE)
    sub = 'Season 2 finale · ship checklist'; bbox = d.textbbox((0, 0), sub, font=font(FONT_REG, 28))
    d.text(((W - (bbox[2] - bbox[0])) // 2, H // 2 + 60), sub, font=font(FONT_REG, 28), fill=BLUE)
    d.text((W // 2 - 90, H // 2 + 150), 'Episode 85', font=font(FONT_BOLD, 30), fill=ORANGE)
    return img.convert("RGB")
...
RENDERERS = {
    ...
    "find": render_find,
    "fix_app": render_fix_app,
    "fix_runtime": render_fix_runtime,
...
def main():
    ...
    (ROOT / "ep84_durations.json").write_text(json.dumps(durations, indent=2))
    outs = [render_scene_clip(sid, r, durations[sid]) for sid, r, _ in SCENES]
    lst = ROOT / "concat_ep84.txt"
    with open(lst, "w") as f:
        for p in outs: f.write(f"file '{p}'\n")
    narrated = OUTPUT / "java_ep84_narrated.mp4"
    ...
    if abs(pace - 1.0) > 0.015:
        ...
        paced = OUTPUT / "java_ep84_paced.mp4"
        ...
        base = paced
    final = OUTPUT / "Java_Episode_84_Performance_Playbook.mp4"
    ...
    srt = OUTPUT / "Java_Episode_84.srt"; write_srt(durations, srt); shutil.copy2(srt, ARTIFACTS / srt.name)
    burned = OUTPUT / "Java_Episode_84_Performance_Playbook_CAPTIONED.mp4"
    ...
    vdir = ARTIFACTS / "ep84_verify"; vdir.mkdir(exist_ok=True)
    for ts, name in [('00:00:12', '01_hook'), ('00:00:50', '02_measure'), ('00:01:40', '03_find'), ('00:02:30', '04_fix'), ('00:03:20', '05_interview')]:
        subprocess.run(["ffmpeg", "-y", "-ss", ts, "-i", str(final), "-frames:v", "1", str(vdir / f"{name}.jpg")], capture_output=True)
    final_dur = probe(final)
    print(f"DONE Episode 84: {final_dur/60:.2f} min")
