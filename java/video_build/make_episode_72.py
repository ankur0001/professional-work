#!/usr/bin/env python3
"""Episode 72 — IoC and Dependency Injection. Narration + visuals authored together."""
...
AUDIO, FRAMES, CLIPS = ROOT / "audio_ep72", ROOT / "frames_ep72", ROOT / "clips_ep72"
VOICE = os.environ.get("KOKORO_VOICE", "am_michael")
SPEED = float(os.environ.get("KOKORO_SPEED", "0.97"))

SCENES = [
    ("hook", "hook", [
        'Episode Seventy-One introduced Spring as a platform around an IoC container.',
        'Inversion of Control flips who constructs collaborators — the framework does.',
        'Dependency Injection is the main technique Spring uses to achieve IoC.',
        'Get this wrong — mysterious nulls, circular dependencies, and untestable code.',
        'Get this right — clear graphs, easy mocks, and predictable startup.',
        'Today — IoC versus DI, injection styles, bean scopes, and wiring pitfalls.',
    ]),
    ("title", "title", [
        'Episode Seventy-Two.',
    ...
    ("ioc_vs_di", "ioc_vs_di", [
        ...
        'Inversion of Control — your code does not new up the whole object graph.',
        'Dependency Injection — dependencies are provided from outside the class.',
        "Spring's ApplicationContext is the injector and lifecycle manager.",
        'Factories, service locators, and events are other IoC styles — DI is the default.',
        'Say IoC for the idea — DI for how Spring usually implements it.',
    ]),
    ("injection_styles", "injection_styles", [
        'Three injection styles you will see in Spring code.',
        'Constructor injection — required dependencies as final fields — preferred.',
        'Setter injection — optional dependencies or reconfiguration after create.',
        'Field injection with Autowired — short, but harder to test and reason about.',
        'Constructor injection makes invariants obvious — object is complete after new.',
        'Modern Spring and Boot samples default to constructor injection for a reason.',
    ]),
    ("scopes", "scopes", [
        'Bean scopes control how many instances the container creates.',
        'Singleton — one shared instance per context — the Spring default.',
        'Prototype — a new instance every time you ask the container.',
        'Request and session scopes — web-aware, tied to HTTP lifecycle.',
        'Wrong scope — shared mutable state across requests is a classic bug.',
        'Default to singleton services with immutable or carefully synchronized state.',
    ]),
    ("wiring", "wiring", [
        'How Spring finds and wires beans.',
        'Component scanning picks up Stereotype annotations — Service, Repository, Controller.',
        'Configuration classes with Bean methods define explicit beans.',
        'Qualifiers disambiguate when multiple candidates share a type.',
        'Profiles activate environment-specific beans — local, staging, prod.',
        'Circular dependencies signal a design smell — break the cycle with redesign.',
    ]),
    ("testing", "testing", [
        'DI pays off hardest in tests.',
        'Unit tests construct the class with mocks — no container required.',
        'Slice tests load a thin Spring context — web or data layer only.',
        'Full ApplicationContext tests catch wiring mistakes — slower but valuable.',
        'Avoid static singletons and ServiceLocator lookups — they fight DI.',
        'If you cannot inject a fake, the design is fighting you.',
    ]),
    ("mistakes", "mistakes", [
        'Three common mistakes.',
        'One — field injection everywhere — invisible dependencies, awkward tests.',
        'Two — singleton beans holding request-specific mutable state.',
        'Three — Autowired on concrete classes with no interface — tight coupling.',
        'Also — ignoring constructor failure — missing beans explode at startup for good reason.',
        'Fail fast at context refresh — never with a NullPointerException in production.',
    ]),
    ("interview", "interview", [
        'Interview question — why prefer constructor injection?',
        'Required dependencies are explicit and can be final.',
        'The object is fully initialized after construction — no half-ready beans.',
        'Unit tests pass mocks without Spring or reflection hacks.',
        'Circular dependencies surface earlier — design problems become visible.',
        'Spring and the community treat constructor injection as the default style.',
    ]),
    ("teaser", "teaser", [
        'Wiring is clear — next is how projects start fast.',
        'Episode Seventy-Three — Spring Boot Basics.',
        'Starters, auto-configuration, configuration properties, and the executable jar.',
        'See you there.',
    ]),
]
...
def render_title(progress, t):
    ...
    for txt, fnt, y, col in [
        ('EPISODE 72', font(FONT_BOLD, 28), H // 2 - 140, mix(BG, MUTED, a)),
        ('IoC and Dependency Injection', font(FONT_SERIF, 48), H // 2 - 30, mix(BG, WHITE, a)),
        ('constructor DI · scopes · wiring', font(FONT_REG, 26), H // 2 + 70, mix(BG, MUTED, a)),
    ]:
        bbox = d.textbbox((0, 0), txt, font=fnt); d.text(((W - (bbox[2] - bbox[0])) // 2, y), txt, font=fnt, fill=col)
    return img.convert("RGB")

def render_ioc_vs_di(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'IoC vs DI', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('IoC', 'framework owns construction', 'ORANGE'), ('DI', 'deps provided from outside', 'BLUE'), ('ApplicationContext', 'injector + lifecycle', 'GREEN')]
...
def render_injection_styles(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Injection Styles', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('constructor', 'required, final, preferred', 'ORANGE'), ('setter', 'optional / reconfigure', 'BLUE'), ('field Autowired', 'short but harder to test', 'GREEN')]
    for i, (k, v, col) in enumerate(items):
        a = ease_out_cubic(clamp((progress - i * 0.18) / 0.3))
        if a <= 0: continue
        y = 220 + i * 200
...
def render_scopes(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Bean Scopes', font=font(FONT_SERIF, 40), fill=WHITE)
    ...
    lines = ['singleton — default, one per context', 'prototype — new each request to container', 'request / session — web lifecycle', 'watch shared mutable state', 'prefer stateless singleton services']
    for i, line in enumerate(lines):
        aa = ease_out_cubic(clamp((progress - i * 0.08) / 0.22))
        d.text((260, 280 + i * 100), line, font=font(FONT_MONO, 26), fill=mix(BG, WHITE, aa))
    return img.convert("RGB")

def render_wiring(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Wiring', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('@Component scan', 'Service · Repository · Controller', 'ORANGE'), ('@Configuration', '@Bean methods explicit', 'BLUE'), ('@Qualifier / profiles', 'disambiguate + env select', 'GREEN')]
    for i, (k, v, col) in enumerate(items):
        a = ease_out_cubic(clamp((progress - i * 0.18) / 0.3))
        if a <= 0: continue
        y = 220 + i * 200
...
def render_testing(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Test Payoff', font=font(FONT_SERIF, 44), fill=WHITE)
    steps = [('unit + mocks', 'ORANGE'), ('slice context', 'BLUE'), ('full context', 'GREEN'), ('no static locator', 'RED')]
...
def render_mistakes(progress, t):
    ...
    items = [('01', 'field injection always', 'prefer constructors'), ('02', 'request state in singleton', 'use request scope / locals'), ('03', 'concrete-only Autowired', 'depend on interfaces')]
...
def render_interview(progress, t):
    ...
    answers = [('explicit deps', 'final required fields', 'ORANGE'), ('fully initialized', 'no half-ready beans', 'BLUE'), ('easy tests', 'pass mocks directly', 'GREEN')]
...
def render_teaser(progress, t):
    ...
    title = 'Spring Boot Basics'; bbox = d.textbbox((0, 0), title, font=font(FONT_SERIF, 42))
    d.text(((W - (bbox[2] - bbox[0])) // 2, H // 2 - 40), title, font=font(FONT_SERIF, 42), fill=WHITE)
    sub = 'starters · auto-config · java -jar'; bbox = d.textbbox((0, 0), sub, font=font(FONT_REG, 28))
    d.text(((W - (bbox[2] - bbox[0])) // 2, H // 2 + 60), sub, font=font(FONT_REG, 28), fill=BLUE)
    d.text((W // 2 - 90, H // 2 + 150), 'Episode 73', font=font(FONT_BOLD, 30), fill=ORANGE)
    return img.convert("RGB")

RENDERERS = {
    "hook": render_hook,
    "title": render_title,
    "ioc_vs_di": render_ioc_vs_di,
    "injection_styles": render_injection_styles,
    "scopes": render_scopes,
    "wiring": render_wiring,
...
def main():
    ...
    (ROOT / "ep72_durations.json").write_text(json.dumps(durations, indent=2))
    outs = [render_scene_clip(sid, r, durations[sid]) for sid, r, _ in SCENES]
    lst = ROOT / "concat_ep72.txt"
    with open(lst, "w") as f:
        for p in outs: f.write(f"file '{p}'\n")
    narrated = OUTPUT / "java_ep72_narrated.mp4"
    ...
    if abs(pace - 1.0) > 0.015:
        ...
        paced = OUTPUT / "java_ep72_paced.mp4"
        ...
        base = paced
    final = OUTPUT / "Java_Episode_72_IoC_and_DI.mp4"
    ...
    srt = OUTPUT / "Java_Episode_72.srt"; write_srt(durations, srt); shutil.copy2(srt, ARTIFACTS / srt.name)
    burned = OUTPUT / "Java_Episode_72_IoC_and_DI_CAPTIONED.mp4"
    ...
    vdir = ARTIFACTS / "ep72_verify"; vdir.mkdir(exist_ok=True)
    for ts, name in [('00:00:12', '01_hook'), ('00:00:50', '02_ioc'), ('00:01:40', '03_styles'), ('00:02:30', '04_scopes'), ('00:03:20', '05_interview')]:
        subprocess.run(["ffmpeg", "-y", "-ss", ts, "-i", str(final), "-frames:v", "1", str(vdir / f"{name}.jpg")], capture_output=True)
    final_dur = probe(final)
    print(f"DONE Episode 72: {final_dur/60:.2f} min")
