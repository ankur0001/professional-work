#!/usr/bin/env python3
"""
4–5 minute condensed Java Episode 1 — Kokoro narration (natural length).
"""
...
AUDIO = ROOT / "audio_short"
CLIPS_IN = ROOT / "clips"
CLIPS_OUT = ROOT / "clips_short"
OUTPUT = Path("/workspace/java/output")
ARTIFACTS = Path("/opt/cursor/artifacts")

VOICE = os.environ.get("KOKORO_VOICE", "am_michael")
SPEED = float(os.environ.get("KOKORO_SPEED", "0.97"))

# Target ~270–290s spoken at speed 0.97
SCENES: list[tuple[str, str, list[str]]] = [
    ...
    (
        ...
        [
            ...
            "If Java doesn't run directly on your computer… what does the JVM do inside memory?",
...
def main():
    ...
    print(f"==> Short cut voice={VOICE} speed={SPEED}")
    ...
    (ROOT / "short_durations.json").write_text(json.dumps(durations, indent=2))
    ...
    # Only tiny pace adjust if barely outside 4–5 min
    pace = 1.0
    if dur > 300:
        pace = min(dur / 295.0, 1.12)
    elif dur < 240:
        pace = max(dur / 245.0, 0.90)
    ...
    if abs(pace - 1.0) > 0.015:
        ...
        paced = OUTPUT / "java_ep1_short_paced.mp4"
    ...
    subprocess.run(
        [
            ...
            "-q:a",
            "2",
            str(ARTIFACTS / "narration_SHORT_preview.mp3"),
        ],
        check=True,
        capture_output=True,
    )
    ...
    assert 230 <= final_dur <= 320, f"Duration {final_dur}s outside 4–5 min window"
