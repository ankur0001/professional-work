#!/usr/bin/env python3
"""Episode 41 — Callable, Future & CompletableFuture. Narration + visuals authored together."""
...
AUDIO, FRAMES, CLIPS = ROOT / "audio_ep41", ROOT / "frames_ep41", ROOT / "clips_ep41"
VOICE = os.environ.get("KOKORO_VOICE", "am_michael")
SPEED = float(os.environ.get("KOKORO_SPEED", "0.97"))

SCENES = [
    ("hook", "hook", [
        'Runnable says do this. But what if the task produces a result?',
        'Callable is Runnable with a return value — and checked exceptions.',
        'Submit a Callable, get a Future — a handle to the eventual result.',
        'Block on get, cancel if needed, check if done.',
        'CompletableFuture takes it further — compose async pipelines.',
        'Today — Callable, Future, and an intro to CompletableFuture.',
    ]),
    ("title", "title", [
        'Episode Forty-One.',
    ...
    ("callable", "callable", [
        'Callable of T is a functional interface — T call throws Exception.',
        'Like Runnable but returns a value and can throw checked exceptions.',
        'Submit to an ExecutorService — the pool runs call on a worker thread.',
        'Use lambdas — Callable task equals open paren close paren arrow compute.',
        'Callable fits any task that produces a result — fetch, parse, calculate.',
        'Separate the computation from where it runs.',
    ]),
    ("future", "future", [
        'Future of T represents a pending result of an asynchronous computation.',
        'get blocks until the result is ready — optionally with a timeout.',
        'get with timeout — wait up to a duration, then throw TimeoutException.',
        'isDone checks completion without blocking. cancel attempts to stop.',
        'cancel with mayInterruptIfRunning — true interrupts a running task.',
        'Always handle ExecutionException — the real cause is in getCause.',
    ]),
    ("completable", "completable", [
        'CompletableFuture of T — a Future you can compose and complete manually.',
        'supplyAsync runs a supplier on the default ForkJoinPool.',
        'thenApply transforms the result. thenCompose chains dependent futures.',
        'allOf and anyOf combine multiple futures.',
        'exceptionally handles failures in the pipeline.',
        'CompletableFuture is the modern way to build async workflows in Java.',
    ]),
    ("vs_runnable", "vs_runnable", [
        'Runnable versus Callable versus Future.',
        'Runnable — void, no checked exceptions, fire-and-forget.',
        'Callable — returns T, throws Exception, submitted for a Future.',
        'Future — read-only view of a pending result.',
        'CompletableFuture — writable, composable, chainable.',
        'Choose based on whether you need a result and how you compose tasks.',
    ]),
    ("when_async", "when_async", [
        'When to use Callable and Future.',
        'Parallel API calls — submit many, collect results with get.',
        'CPU work off the request thread — return Future to caller.',
        'Batch processing where each unit produces output.',
        'CompletableFuture when you need chaining — thenApply, thenCombine.',
        'When not — simple fire-and-forget — Runnable and execute suffice.',
    ]),
    ("mistakes", "mistakes", [
        'Three common mistakes.',
        'One — calling get on the event loop thread — blocks the UI.',
        'Two — ignoring ExecutionException — swallowing the real error.',
        'Three — not setting timeouts on get — waits forever on hung tasks.',
        'Also — chaining blocking gets instead of thenCompose.',
        'Async code needs async thinking — do not block what should stay free.',
    ]),
    ("interview", "interview", [
        'Interview question — Callable versus Runnable?',
        'Runnable returns void, no checked exceptions.',
        'Callable returns a value and can throw checked exceptions.',
        'Submit Callable to ExecutorService — receive Future of T.',
        'Future get blocks for result — use timeout in production.',
        'Mention CompletableFuture for composable async pipelines.',
    ]),
    ("teaser", "teaser", [
        'Tasks and futures coordinate work. What about shared data structures?',
        'Episode Forty-Two — Concurrent Collections.',
        'ConcurrentHashMap, CopyOnWriteArrayList, and thread-safe queues.',
        'See you there.',
    ]),
]
...
def render_hook(progress, t):
    ...
    for i,(lab,col) in enumerate([("Callable",ORANGE),("Future",BLUE),("result",GREEN)]):
        ...
        d.text((x+90,520), lab, font=font(FONT_BOLD,30), fill=mix(BG,col,a))
        if i<2:
...
def render_title(progress, t):
    ...
    for txt,fnt,y,col in [
        ("EPISODE 41", font(FONT_BOLD,28), H//2-140, mix(BG,MUTED,a)),
        ("Callable & Future", font(FONT_SERIF,58), H//2-30, mix(BG,WHITE,a)),
        ("get() · CompletableFuture · compose", font(FONT_REG,28), H//2+70, mix(BG,MUTED,a)),
    ]:
        bbox=d.textbbox((0,0),txt,font=fnt); d.text(((W-(bbox[2]-bbox[0]))//2,y),txt,font=fnt,fill=col)
    return img.convert("RGB")

def render_callable(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "Callable<T>", font=font(FONT_SERIF,46), fill=WHITE)
    ...
    lines=["Callable<String> task = () -> fetchData();","Future<String> future = pool.submit(task);","// call() returns T, throws Exception"]
    for i,line in enumerate(lines):
        aa=ease_out_cubic(clamp((progress-i*0.12)/0.28))
        d.text((280,320+i*140), line, font=font(FONT_MONO,28), fill=mix(BG,WHITE,aa))
    return img.convert("RGB")

def render_future(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "Future<T>", font=font(FONT_SERIF,48), fill=WHITE)
    items=[("get()","block for result",ORANGE),("get(timeout)","wait with limit",BLUE),("cancel()","stop if possible",GREEN)]
...
def render_completable(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "CompletableFuture", font=font(FONT_SERIF,44), fill=WHITE)
    a=ease_out_cubic(clamp(progress/0.4))
    d.rounded_rectangle([180,200,1740,840], radius=16, fill=mix(BG,SURFACE,a), outline=mix(BG,GREEN,a), width=3)
    lines=["CompletableFuture.supplyAsync(() -> fetch())","  .thenApply(data -> parse(data))","  .thenCompose(r -> saveAsync(r))","  .exceptionally(ex -> fallback());"]
    for i,line in enumerate(lines):
        aa=ease_out_cubic(clamp((progress-i*0.12)/0.28))
        d.text((280,280+i*120), line, font=font(FONT_MONO,26), fill=mix(BG,WHITE,aa))
    return img.convert("RGB")

def render_vs_runnable(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "Runnable vs Callable", font=font(FONT_SERIF,44), fill=WHITE)
    ...
    if left>0:
        ...
        d.text((220,280), "Runnable", font=font(FONT_BOLD,32), fill=mix(BG,ORANGE,left))
        for i,lab in enumerate(["void run()","no checked exceptions","fire-and-forget"]):
    ...
    if right>0:
        ...
        d.text((1100,280), "Callable<T>", font=font(FONT_BOLD,32), fill=mix(BG,BLUE,right))
        for i,lab in enumerate(["T call()","throws Exception","returns Future<T>"]):
            d.text((1120,380+i*70), lab, font=font(FONT_REG,28), fill=mix(BG,WHITE,right))
    return img.convert("RGB")

def render_when_async(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "When Async Results", font=font(FONT_SERIF,46), fill=WHITE)
    good=[("parallel API calls",GREEN),("offload CPU from request thread",ORANGE),("chain with CompletableFuture",BLUE)]
    ...
    if a>0:
        ...
        d.text((280,760), "✗  Simple fire-and-forget — Runnable is enough", font=font(FONT_BOLD,28), fill=mix(BG,RED,a))
    return img.convert("RGB")

def render_mistakes(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,70), "Common Mistakes", font=font(FONT_SERIF,48), fill=WHITE)
    items=[("01","get() on UI thread","blocks event loop"),("02","ignore ExecutionException","miss real error"),("03","no timeout on get()","waits forever")]
...
def render_interview(progress, t):
    ...
    answers=[("Runnable","void, no checked exceptions",ORANGE),("Callable","returns T, throws Exception",BLUE),("Future","get() for result, use timeout",GREEN)]
...
def render_teaser(progress, t):
    ...
    title="Concurrent Collections"; bbox=d.textbbox((0,0),title,font=font(FONT_SERIF,50))
    d.text(((W-(bbox[2]-bbox[0]))//2, H//2-40), title, font=font(FONT_SERIF,50), fill=WHITE)
    sub="ConcurrentHashMap · thread-safe queues"; bbox=d.textbbox((0,0),sub,font=font(FONT_REG,28))
    d.text(((W-(bbox[2]-bbox[0]))//2, H//2+60), sub, font=font(FONT_REG,28), fill=BLUE)
    d.text((W//2-90, H//2+150), "Episode 42", font=font(FONT_BOLD,30), fill=ORANGE)
    return img.convert("RGB")

RENDERERS = {"hook": render_hook, "title": render_title, "callable": render_callable, "future": render_future, "completable": render_completable, "vs_runnable": render_vs_runnable, "when_async": render_when_async, "mistakes": render_mistakes, "interview": render_interview, "teaser": render_teaser}
...
def main():
    ...
    (ROOT / "ep41_durations.json").write_text(json.dumps(durations, indent=2))
    outs = [render_scene_clip(sid, r, durations[sid]) for sid, r, _ in SCENES]
    lst = ROOT / "concat_ep41.txt"
    with open(lst, "w") as f:
        for p in outs: f.write(f"file '{p}'\n")
    narrated = OUTPUT / "java_ep41_narrated.mp4"
    ...
    if abs(pace - 1.0) > 0.015:
        ...
        paced = OUTPUT / "java_ep41_paced.mp4"
        ...
        base = paced
    final = OUTPUT / "Java_Episode_41_Callable_Future.mp4"
    ...
    srt = OUTPUT / "Java_Episode_41.srt"; write_srt(durations, srt); shutil.copy2(srt, ARTIFACTS / srt.name)
    burned = OUTPUT / "Java_Episode_41_Callable_Future_CAPTIONED.mp4"
    ...
    vdir = ARTIFACTS / "ep41_verify"; vdir.mkdir(exist_ok=True)
    for ts, name in [('00:00:12', '01_hook'), ('00:00:50', '02_callable'), ('00:01:40', '03_future'), ('00:02:30', '04_completable'), ('00:03:20', '05_interview')]:
        subprocess.run(["ffmpeg","-y","-ss",ts,"-i",str(final),"-frames:v","1",str(vdir/f"{name}.jpg")], capture_output=True)
    final_dur = probe(final)
    print(f"DONE Episode 41: {final_dur/60:.2f} min")
