#!/usr/bin/env python3
"""Episode 26 — Streams Intro. Narration + visuals authored together."""
...
AUDIO, FRAMES, CLIPS = ROOT / "audio_ep26", ROOT / "frames_ep26", ROOT / "clips_ep26"
VOICE = os.environ.get("KOKORO_VOICE", "am_michael")
SPEED = float(os.environ.get("KOKORO_SPEED", "0.97"))

SCENES = [
    ("hook", "hook", [
        'Sorting orders a collection. Streams transform how you process one.',
        'Instead of nested loops everywhere — declare a pipeline.',
        'Source, intermediate operations, terminal operation.',
        'Lazy until it needs to run. Expressive when the problem fits.',
        'Today — the mental model that makes Streams useful.',
        'Data as a pipeline — not a pile of index variables.',
    ]),
    ("title", "title", [
        'Episode Twenty-Six.',
        'Streams introduction — pipelines over data.',
    ]),
    ("idea", "idea", [
        'A Stream is not a new collection type.',
        'It is a sequence of elements supporting aggregate operations.',
        'You build a pipeline — then a terminal operation triggers computation.',
        'Streams are single-use. Consume once.',
        'They can come from collections, arrays, generators, or I/O wrappers.',
        'Think recipe first — result second.',
    ]),
    ("ops", "ops", [
        'Operations split into two families.',
        'Intermediate — filter, map, flatMap, sorted, distinct — return a stream.',
        'Terminal — collect, reduce, forEach, count, anyMatch — produce a result or side effect.',
        'Without a terminal operation, nothing useful happens.',
        'Keep intermediate steps free of surprising mutation.',
        'Readable pipelines beat clever one-liners.',
    ]),
    ("lazy", "lazy", [
        'Laziness is the key performance idea.',
        'Intermediate operations record what to do — they do not run yet.',
        'A terminal operation pulls data through the pipeline.',
        'anyMatch can stop early. limit can bound work.',
        'That is why filter then findFirst can skip unused elements.',
        'Use laziness — do not fight it with eager side effects mid-pipeline.',
    ]),
    ("collect", "collect", [
        'collect is how most pipelines finish.',
        'Collectors.toList and toSet materialize results.',
        'toMap builds maps carefully — watch duplicate keys.',
        'groupingBy clusters elements by a classifier.',
        'joining builds strings without manual StringBuilder noise.',
        'Choose a collector that matches the shape you need next.',
    ]),
    ("when", "when", [
        'When Streams shine.',
        'Transform and filter chains that would be noisy loops.',
        'Pipelines that read like the business rule.',
        'Optional parallelism later — after you measure.',
        'When not — heavy mutable accumulation that is clearer as a for-loop.',
        'Clarity first. Streams are a tool, not a purity contest.',
    ]),
    ("mistakes", "mistakes", [
        'Three common mistakes.',
        'One — reusing a stream after it was consumed.',
        'Two — sneaking side effects into map instead of keeping transformations pure.',
        'Three — sprinkling parallel without evidence it helps.',
        'Also — giant pipelines nobody can debug at a breakpoint.',
        'Good stream code is still boringly clear.',
    ]),
    ("interview", "interview", [
        'Interview question — what is a Stream, and why does laziness matter?',
        'A pipeline over elements with intermediate and terminal operations.',
        'Laziness delays work until a terminal operation — enabling short-circuiting.',
        'Mention collect as the common way to materialize results.',
        'Contrast with collections — streams do not store elements themselves.',
        'That answer shows conceptual understanding.',
    ]),
    ("teaser", "teaser", [
        'Pipelines are in place. Next — collectors with real shape.',
        'Episode Twenty-Seven — Stream Collectors.',
...
def render_hook(progress, t):
    ...
    for i,(lab,col) in enumerate(stages):
        ...
        d.text((x+90,530), lab, font=font(FONT_BOLD,32), fill=mix(BG,col,a))
        if i<3:
...
def render_title(progress, t):
    ...
    for txt,fnt,y,col in [
        ("EPISODE 26", font(FONT_BOLD,28), H//2-140, mix(BG,MUTED,a)),
        ("Streams Intro", font(FONT_SERIF,68), H//2-30, mix(BG,WHITE,a)),
        ("pipeline · lazy · collect", font(FONT_REG,30), H//2+70, mix(BG,MUTED,a)),
    ]:
...
def render_idea(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "What a Stream Is", font=font(FONT_SERIF,46), fill=WHITE)
    items=[("not a data structure","a view over a sequence of elements",ORANGE),("pipeline", "source → intermediate → terminal",BLUE),("one shot","consume once, then done",GREEN)]
...
def render_ops(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "Core Operations", font=font(FONT_SERIF,46), fill=WHITE)
    left=ease_out_cubic(clamp(progress/0.4)); right=ease_out_cubic(clamp((progress-0.3)/0.4))
    if left>0:
        d.rounded_rectangle([140,220,900,820], radius=18, fill=mix(BG,SURFACE,left), outline=mix(BG,BLUE,left), width=4)
        d.text((220,280), "Intermediate", font=font(FONT_BOLD,32), fill=mix(BG,BLUE,left))
        for i,lab in enumerate(["filter","map","flatMap","sorted","distinct"]):
            d.text((240,380+i*70), lab, font=font(FONT_MONO,28), fill=mix(BG,WHITE,left))
    if right>0:
        d.rounded_rectangle([1020,220,1780,820], radius=18, fill=mix(BG,SURFACE,right), outline=mix(BG,ORANGE,right), width=4)
        d.text((1100,280), "Terminal", font=font(FONT_BOLD,32), fill=mix(BG,ORANGE,right))
        for i,lab in enumerate(["collect","forEach","reduce","count","anyMatch"]):
            d.text((1120,380+i*70), lab, font=font(FONT_MONO,28), fill=mix(BG,WHITE,right))
    return img.convert("RGB")

def render_lazy(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "Laziness", font=font(FONT_SERIF,48), fill=WHITE)
    ...
    lines=["Intermediate ops build a recipe","Nothing runs until a terminal op","Short-circuit ops can stop early"]
    for i,line in enumerate(lines):
        aa=ease_out_cubic(clamp((progress-i*0.15)/0.3))
        d.text((320,360+i*120), line, font=font(FONT_REG,32), fill=mix(BG,WHITE,aa))
    return img.convert("RGB")

def render_collect(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "Collect to Results", font=font(FONT_SERIF,46), fill=WHITE)
    a=ease_out_cubic(clamp(progress/0.4))
    d.rounded_rectangle([180,200,1740,840], radius=16, fill=mix(BG,SURFACE,a), outline=mix(BG,ORANGE,a), width=3)
    lines=["collect(Collectors.toList())","toSet() / toMap(...)","groupingBy(classifier)","joining(\", \") for strings"]
    for i,line in enumerate(lines):
...
def render_when(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "When Streams Shine", font=font(FONT_SERIF,46), fill=WHITE)
    good=[("Transform + filter chains",GREEN),("Readable data pipelines",ORANGE),("Optional parallel later",BLUE)]
    ...
    if a>0:
        ...
        d.text((280,760), "✗  Simple loops with heavy mutable local state", font=font(FONT_BOLD,28), fill=mix(BG,RED,a))
    return img.convert("RGB")

def render_mistakes(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,70), "Common Mistakes", font=font(FONT_SERIF,48), fill=WHITE)
    items=[("01","Reuse a consumed stream","Create a new stream"),("02","Side effects in map","Keep map pure; use forEach carefully"),("03","parallel() by default","Measure; sequential is fine")]
...
def render_interview(progress, t):
    ...
    q="What is a Stream, and why laziness?"
    bbox=d.textbbox((0,0),q,font=font(FONT_BOLD,32)); d.text(((W-(bbox[2]-bbox[0]))//2,190),q,font=font(FONT_BOLD,32),fill=WHITE)
    answers=[("Stream","pipeline over elements",ORANGE),("Lazy","runs at terminal op",BLUE),("Collect","materialize results",GREEN)]
...
def render_teaser(progress, t):
    ...
    title="Stream Collectors"; bbox=d.textbbox((0,0),title,font=font(FONT_SERIF,58))
    d.text(((W-(bbox[2]-bbox[0]))//2, H//2-40), title, font=font(FONT_SERIF,58), fill=WHITE)
    sub="grouping · partitioning · downstream"; bbox=d.textbbox((0,0),sub,font=font(FONT_REG,30))
    d.text(((W-(bbox[2]-bbox[0]))//2, H//2+60), sub, font=font(FONT_REG,30), fill=BLUE)
    d.text((W//2-90, H//2+150), "Episode 27", font=font(FONT_BOLD,30), fill=ORANGE)
    return img.convert("RGB")


RENDERERS = {"hook": render_hook, "title": render_title, "idea": render_idea, "ops": render_ops, "lazy": render_lazy, "collect": render_collect, "when": render_when, "mistakes": render_mistakes, "interview": render_interview, "teaser": render_teaser}
...
def main():
    ...
    (ROOT / "ep26_durations.json").write_text(json.dumps(durations, indent=2))
    outs = [render_scene_clip(sid, r, durations[sid]) for sid, r, _ in SCENES]
    lst = ROOT / "concat_ep26.txt"
    with open(lst, "w") as f:
        for p in outs: f.write(f"file '{p}'\n")
    narrated = OUTPUT / "java_ep26_narrated.mp4"
    ...
    if abs(pace - 1.0) > 0.015:
        ...
        paced = OUTPUT / "java_ep26_paced.mp4"
        ...
        base = paced
    final = OUTPUT / "Java_Episode_26_Streams_Intro.mp4"
    ...
    srt = OUTPUT / "Java_Episode_26.srt"; write_srt(durations, srt); shutil.copy2(srt, ARTIFACTS / srt.name)
    burned = OUTPUT / "Java_Episode_26_Streams_Intro_CAPTIONED.mp4"
    ...
    vdir = ARTIFACTS / "ep26_verify"; vdir.mkdir(exist_ok=True)
    ...
    print(f"DONE Episode 26: {final_dur/60:.2f} min")
