...
set -euo pipefail
cd /workspace
MAP_PY='java/video_build/distribute_v2_to_episode_prs.py'
OUT=java/output/v2
mkdir -p "$OUT"
...
# prefer output/v2 then java/output/v2
...
for ep in $(seq "$START" "$END"); do
  ...
  python3 -u java/video_build/render_v2_from_narrative.py --ep "$ep" --reuse-audio
