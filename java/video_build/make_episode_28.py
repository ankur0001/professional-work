#!/usr/bin/env python3
"""Episode 28 — flatMap & Composition. Narration + visuals authored together."""
...
AUDIO, FRAMES, CLIPS = ROOT / "audio_ep28", ROOT / "frames_ep28", ROOT / "clips_ep28"
VOICE = os.environ.get("KOKORO_VOICE", "am_michael")
SPEED = float(os.environ.get("KOKORO_SPEED", "0.97"))

SCENES = [
    ("hook", "hook", [
        'Collectors gave results shape. Now expand elements inside the pipeline.',
        'One input can become many outputs — without nested loops.',
        'flatMap is map plus flatten — the one-to-many transform.',
        'Composition chains small steps into readable pipelines.',
        'Today — flatten complexity instead of hiding it in loops.',
        'Streams express structure. flatMap expresses expansion.',
    ]),
    ("title", "title", [
        'Episode Twenty-Eight.',
        'flatMap and composition — one-to-many pipelines.',
    ]),
    ("flatmap", "flatmap", [
        'map transforms one element into one result.',
        'flatMap transforms one element into a stream of results.',
        'The stream is flattened into the parent pipeline.',
        'List of lists becomes a single flat sequence.',
        'Optional values become present elements — absent ones drop away.',
        'Think expand, then merge — not nested for-each.',
    ]),
    ("composition", "composition", [
        'Composition means chaining focused operations.',
        'Each step does one job — filter, map, flatMap, collect.',
        'Read pipelines top to bottom like a sentence.',
        'Extract a method when a chain grows hard to name.',
        'Good composition favors clarity over cleverness.',
        'Small steps compose into big behavior.',
    ]),
    ("nested", "nested", [
        'Without flatMap, nested structures invite nested loops.',
        'Orders with line items. Departments with employees.',
        'A map gives you a stream of collections — still nested.',
        'flatMap unwraps each inner collection into the outer flow.',
        'One pipeline replaces index juggling.',
        'Flatten at the right level — not too early, not too late.',
    ]),
    ("onetomany", "onetomany", [
        'One-to-many shows up everywhere in real domains.',
        'Split a sentence into words. Parse CSV fields.',
        'Expand a user into their roles or permissions.',
        'flatMap with Arrays.stream or Collection.stream is idiomatic.',
        'Choose flatMap when the natural result is many, not one.',
        'If you only need one, map is simpler.',
    ]),
    ("patterns", "patterns", [
        'Common patterns worth memorizing.',
        'flatMap(Optional::stream) drops empty optionals cleanly.',
        'flatMap(Collection::stream) flattens nested collections.',
        'flatMap(s -> s.lines()) splits text into lines.',
        'distinct and sorted still apply after flattening.',
        'Compose terminal collectors at the end — shape stays last.',
    ]),
    ("mistakes", "mistakes", [
        'Three common mistakes.',
        'One — using map when flatMap is required — you get Stream of Stream.',
        'Two — flattening too eagerly and losing grouping context.',
        'Three — giant flatMap lambdas that should be named methods.',
        'Also — forgetting that order is preserved in sequential streams.',
        'Readable steps beat one opaque flatMap block.',
    ]),
    ("interview", "interview", [
        'Interview question — map versus flatMap?',
        'map — one input, one output element in the stream.',
        'flatMap — one input, zero or more outputs, flattened.',
        'Give examples — words from a line, items from an order.',
        'Mention Optional::stream for filtering absent values.',
        'That answer shows you understand stream geometry.',
    ]),
    ("teaser", "teaser", [
        'Pipelines flatten nicely. Next — parallelism with care.',
        'Episode Twenty-Nine — Parallel Streams.',
        'When fork-join helps — and when it hurts.',
        'See you there.',
    ]),
]
...
def render_title(progress, t):
    ...
    for txt,fnt,y,col in [
        ("EPISODE 28", font(FONT_BOLD,28), H//2-140, mix(BG,MUTED,a)),
        ("flatMap & Composition", font(FONT_SERIF,54), H//2-30, mix(BG,WHITE,a)),
        ("one-to-many · flatten · compose", font(FONT_REG,28), H//2+70, mix(BG,MUTED,a)),
    ]:
        bbox=d.textbbox((0,0),txt,font=fnt); d.text(((W-(bbox[2]-bbox[0]))//2,y),txt,font=fnt,fill=col)
    return img.convert("RGB")

def render_flatmap(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    ...
    if left>0:
        ...
        d.text((240,380), "T → R", font=font(FONT_MONO,36), fill=mix(BG,WHITE,left))
        d.text((240,480), "one in, one out", font=font(FONT_REG,28), fill=mix(BG,MUTED,left))
    if right>0:
        ...
        d.text((1120,380), "T → Stream<R>", font=font(FONT_MONO,30), fill=mix(BG,WHITE,right))
        d.text((1120,480), "flattened into parent", font=font(FONT_REG,28), fill=mix(BG,MUTED,right))
    return img.convert("RGB")

def render_composition(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "Pipeline Composition", font=font(FONT_SERIF,44), fill=WHITE)
    steps=[("filter",ORANGE),("map",BLUE),("flatMap",GREEN),("collect",MUTED)]
...
def render_nested(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "Flatten Nested Data", font=font(FONT_SERIF,44), fill=WHITE)
    ...
    d.text((240,270), "List<Order> → flatMap(order → items)", font=font(FONT_MONO,30), fill=mix(BG,WHITE,a))
    for i,(k,v) in enumerate([("[A,B]", "nested"),("[C,D,E]", "nested"),("flat A,B,C,D,E", "one stream")]):
        ...
        d.text((x+80,580), k, font=font(FONT_MONO,28), fill=mix(BG,BLUE,aa))
        d.text((x+120,680), v, font=font(FONT_REG,28), fill=mix(BG,WHITE,aa))
    return img.convert("RGB")

def render_onetomany(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "One-to-Many Examples", font=font(FONT_SERIF,44), fill=WHITE)
    rows=[("split lines","text → words",ORANGE),("order items","order → line items",BLUE),("user roles","user → permissions",GREEN)]
...
def render_patterns(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "Idiomatic Patterns", font=font(FONT_SERIF,44), fill=WHITE)
    ...
    lines=["flatMap(Optional::stream)","flatMap(Collection::stream)","flatMap(s -> s.lines())","then distinct / sorted / collect"]
...
def render_mistakes(progress, t):
    ...
    items=[("01","map instead of flatMap","Stream of Stream — use flatMap"),("02","flatten too early","Lose grouping context"),("03","giant flatMap lambda","Extract a named method")]
...
def render_interview(progress, t):
    ...
    q="map vs flatMap — when each?"
    bbox=d.textbbox((0,0),q,font=font(FONT_BOLD,32)); d.text(((W-(bbox[2]-bbox[0]))//2,190),q,font=font(FONT_BOLD,32),fill=WHITE)
    answers=[("map","one in, one out",ORANGE),("flatMap","zero or many, flattened",BLUE),("Example","items from orders",GREEN)]
...
def render_teaser(progress, t):
    ...
    title="Parallel Streams"; bbox=d.textbbox((0,0),title,font=font(FONT_SERIF,58))
    d.text(((W-(bbox[2]-bbox[0]))//2, H//2-40), title, font=font(FONT_SERIF,58), fill=WHITE)
    sub="fork-join · pools · when to parallelize"; bbox=d.textbbox((0,0),sub,font=font(FONT_REG,28))
    d.text(((W-(bbox[2]-bbox[0]))//2, H//2+60), sub, font=font(FONT_REG,28), fill=BLUE)
    d.text((W//2-90, H//2+150), "Episode 29", font=font(FONT_BOLD,30), fill=ORANGE)
    return img.convert("RGB")

RENDERERS = {"hook": render_hook, "title": render_title, "flatmap": render_flatmap, "composition": render_composition, "nested": render_nested, "onetomany": render_onetomany, "patterns": render_patterns, "mistakes": render_mistakes, "interview": render_interview, "teaser": render_teaser}
...
def main():
    ...
    (ROOT / "ep28_durations.json").write_text(json.dumps(durations, indent=2))
    outs = [render_scene_clip(sid, r, durations[sid]) for sid, r, _ in SCENES]
    lst = ROOT / "concat_ep28.txt"
    with open(lst, "w") as f:
        for p in outs: f.write(f"file '{p}'\n")
    narrated = OUTPUT / "java_ep28_narrated.mp4"
    ...
    if abs(pace - 1.0) > 0.015:
        ...
        paced = OUTPUT / "java_ep28_paced.mp4"
        ...
        base = paced
    final = OUTPUT / "Java_Episode_28_FlatMap_Composition.mp4"
    ...
    srt = OUTPUT / "Java_Episode_28.srt"; write_srt(durations, srt); shutil.copy2(srt, ARTIFACTS / srt.name)
    burned = OUTPUT / "Java_Episode_28_FlatMap_Composition_CAPTIONED.mp4"
    ...
    vdir = ARTIFACTS / "ep28_verify"; vdir.mkdir(exist_ok=True)
    for ts, name in [('00:00:12', '01_hook'), ('00:00:50', '02_flatmap'), ('00:01:40', '03_nested'), ('00:02:30', '04_patterns'), ('00:03:20', '05_interview')]:
        subprocess.run(["ffmpeg","-y","-ss",ts,"-i",str(final),"-frames:v","1",str(vdir/f"{name}.jpg")], capture_output=True)
    final_dur = probe(final)
    print(f"DONE Episode 28: {final_dur/60:.2f} min")
