...
1. In the last episode, we worked through Safepoints.
2. If that made sense, today builds on it. If it was fuzzy, today usually makes it click.
3. Startup is a feature — classloading and init dominate cold starts.
...
2. Picture JVM Startup clearly.
3. Here are the points that matter when code meets production:
4. Point 1: Classloading + verification + init.
5. If you remember only one thing, make it that.
6. Point 2: CDS/AppCDS.
7. This is usually where tutorials stop — we will not.
8. Point 3: AOT / Graal native image trade-offs.
9. Point 4: CRaC for snapshotted restarts.
10. Point 5: Lazy init carefully.
...
5. Look at `java -jar app.jar`.
6. Comment cue: measure cold start separately from steady-state
...
3. With JVM Startup, mastery is not more jargon.
...
2. Mistake 1: Doing heavy work in static initializers.
3. I have seen this in real code reviews — including my own older code.
4. Mistake 2: Ignoring class path bloat.
5. I have seen this in real code reviews — including my own older code.
6. Mistake 3: Optimizing startup without measuring.
...
2. Question: Ways to improve startup?
3. Answer: Reduce classpath work, CDS, lazy init, native image where it fits.
...
2. Classloading + verification + init.
...
10. AOT / Graal native image trade-offs.
...
14. CRaC for snapshotted restarts.
...
3. 1) Say out loud what JVM Startup is for in one sentence.
...
2. Episode 66: JVM Interview Wrap.
