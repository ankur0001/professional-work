...
1. In the last episode, we worked through ForkJoinPool.
2. If that made sense, today builds on it. If it was fuzzy, today usually makes it click.
3. ThreadLocal is per-thread global state — powerful and leak-prone.
...
2. Picture ThreadLocal clearly before edge cases.
3. Here are the points that matter when code meets production:
4. Point 1: withInitial helpers.
5. If you remember only one thing, make it that.
6. Point 2: Always remove in request-scoped pool usage.
7. This is usually where tutorials stop — we will not.
8. Point 3: Harder reasoning than passing context.
9. Point 4: Virtual threads multiply instances.
10. Point 5: Prefer explicit context parameters when possible.
...
5. Look at `static final ThreadLocal<SimpleDateFormat> FMT =`.
6. That is not decorative syntax — it encodes a real rule of the platform or API.
7. Look at `ThreadLocal.withInitial(() -> new SimpleDateFormat("yyyy"));`.
...
3. With ThreadLocal, mastery is not more jargon.
...
2. Mistake 1: Forgetting remove() in thread pools.
3. I have seen this in real code reviews — including my own older code.
4. Mistake 2: Using ThreadLocal as a hidden API.
5. I have seen this in real code reviews — including my own older code.
6. Mistake 3: Sharing ThreadLocal values across threads somehow.
...
2. Question: Classic ThreadLocal leak?
3. Answer: Values stick to reused pool threads — remove in finally.
...
6. Always remove in request-scoped pool usage.
...
10. Harder reasoning than passing context.
...
14. Virtual threads multiply instances.
...
18. Prefer explicit context parameters when possible.
...
3. 1) Say out loud what ThreadLocal is for in one sentence.
