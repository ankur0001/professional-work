...
1. In the last episode, we worked through API Design Deep Dive.
2. If that made sense, today builds on it. If it was fuzzy, today usually makes it click.
3. Events are facts — consumers must tolerate at-least-once delivery.
...
2. Picture Event-Driven Architecture clearly.
3. Here are the points that matter when code meets production:
4. Point 1: Facts vs commands.
5. If you remember only one thing, make it that.
6. Point 2: Idempotent consumers.
7. This is usually where tutorials stop — we will not.
8. Point 3: Outbox pattern.
9. Point 4: Ordering is partial.
10. Point 5: Schema evolution.
...
5. Comment cue: transactionally write Order + OutboxEvent
6. Comment cue: publisher relays outbox to the bus
7. Comment cue: consumers handle OrderCreated idempotently
...
3. With Event-Driven Architecture, mastery is not more jargon.
...
2. Question: Why outbox?
3. Answer: Atomically record state change + event to avoid lost messages.
...
3. 1) Say out loud what Event-Driven Architecture is for in one sentence.
...
1. Landing the plane.
2. Today was Event-Driven Architecture.
...
2. Episode 84: Performance Playbook.
