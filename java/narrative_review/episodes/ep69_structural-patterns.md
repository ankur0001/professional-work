...
1. In the last episode, we worked through Creational Patterns.
2. If that made sense, today builds on it. If it was fuzzy, today usually makes it click.
3. Structural patterns assemble objects — adapters, decorators, facades, proxies.
...
2. Picture Structural Patterns clearly.
3. Here are the points that matter when code meets production:
4. Point 1: Composition wraps behavior.
5. If you remember only one thing, make it that.
6. Point 2: Decorator adds without subclass explosion.
7. This is usually where tutorials stop — we will not.
8. Point 3: Facade simplifies subsystems.
9. Point 4: Proxy controls access.
10. Point 5: Spring AOP uses proxies.
...
5. Look at `class LoggingRepo implements Repo {`.
6. That is not decorative syntax — it encodes a real rule of the platform or API.
7. Look at `private final Repo delegate;`.
8. Look at `public void save(Entity e) { log(e); delegate.save(e); }`.
...
3. With Structural Patterns, mastery is not more jargon.
...
2. Question: Decorator vs inheritance?
3. Answer: Decorator adds behavior via composition without exploding subclasses.
...
6. Decorator adds without subclass explosion.
...
10. Facade simplifies subsystems.
...
3. 1) Say out loud what Structural Patterns is for in one sentence.
...
1. Landing the plane.
2. Today was Structural Patterns.
...
2. Episode 70: Behavioral Patterns.
