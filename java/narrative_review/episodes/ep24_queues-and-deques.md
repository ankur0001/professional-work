...
1. In the last episode, we worked through Maps.
2. If that made sense, today builds on it. If it was fuzzy, today usually makes it click.
3. Queues shape producer/consumer flow — Deque is the modern versatile choice.
...
2. Picture Queues and Deques clearly before edge cases.
3. Here are the points that matter when code meets production:
4. Point 1: FIFO vs double-ended access.
5. If you remember only one thing, make it that.
6. Point 2: ArrayDeque is a strong general default.
7. This is usually where tutorials stop — we will not.
8. Point 3: offer/poll vs add/remove exception behavior.
9. Point 4: Legacy Stack — prefer Deque.
10. Point 5: Blocking queues appear in concurrency.
...
5. Look at `Deque<String> q = new ArrayDeque<>();`.
6. That is not decorative syntax — it encodes a real rule of the platform or API.
7. Look at `q.offerLast("a");`.
8. Look at `String x = q.pollFirst();`.
...
3. With Queues and Deques, mastery is not more jargon.
...
2. Mistake 1: Using Stack legacy class.
3. I have seen this in real code reviews — including my own older code.
4. Mistake 2: Busy-waiting instead of blocking structures later.
5. I have seen this in real code reviews — including my own older code.
6. Mistake 3: Ignoring capacity/backpressure needs.
...
2. Question: Why ArrayDeque over Stack?
3. Answer: Clearer API and better performance than legacy Vector-based Stack.
...
6. ArrayDeque is a strong general default.
...
10. offer/poll vs add/remove exception behavior.
...
18. Blocking queues appear in concurrency.
...
3. 1) Say out loud what Queues and Deques is for in one sentence.
...
2. Episode 25: Sorting and Comparators.
