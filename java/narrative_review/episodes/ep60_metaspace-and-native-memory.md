...
1. In the last episode, we worked through Escape Analysis.
2. If that made sense, today builds on it. If it was fuzzy, today usually makes it click.
3. -Xmx is not your process size — native and metaspace count.
...
2. Picture Metaspace and Native Memory clearly.
3. Here are the points that matter when code meets production:
4. Point 1: Metaspace for class metadata.
5. If you remember only one thing, make it that.
6. Point 2: Direct buffers, thread stacks, JNI, GC native.
7. This is usually where tutorials stop — we will not.
8. Point 3: Container limits need headroom.
9. Point 4: Native memory tracking helps.
10. Point 5: Classloader leaks show in metaspace.
...
5. Look at `java -Xmx512m -XX:MaxMetaspaceSize=256m -jar app.jar`.
6. Comment cue: still leave room for threads/native/direct buffers
...
3. With Metaspace and Native Memory, mastery is not more jargon.
...
2. Question: Why die with free heap?
3. Answer: Native/metaspace/stacks/container limits beyond -Xmx.
...
2. Metaspace for class metadata.
...
6. Direct buffers, thread stacks, JNI, GC native.
...
10. Container limits need headroom.
...
14. Native memory tracking helps.
...
18. Classloader leaks show in metaspace.
...
3. 1) Say out loud what Metaspace and Native Memory is for in one sentence.
...
1. Landing the plane.
2. Today was Metaspace and Native Memory.
