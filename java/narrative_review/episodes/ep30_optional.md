...
1. In the last episode, we worked through Parallel Streams.
2. If that made sense, today builds on it. If it was fuzzy, today usually makes it click.
3. Optional makes absence explicit at API boundaries — not everywhere.
...
2. Picture Optional clearly before edge cases.
3. Here are the points that matter when code meets production:
4. Point 1: Prefer as return type.
5. If you remember only one thing, make it that.
6. Point 2: map/flatMap/filter.
7. This is usually where tutorials stop — we will not.
8. Point 3: orElse vs orElseGet.
9. Point 4: Avoid Optional.of(null).
10. Point 5: Don't use Optional fields everywhere.
...
5. Look at `Optional<User> u = find();`.
6. That is not decorative syntax — it encodes a real rule of the platform or API.
7. Look at `String name = u.map(User::name).orElse("unknown");`.
...
3. With Optional, mastery is not more jargon.
...
2. Question: Why not Optional as a field?
3. Answer: Noise and allocation; use it to communicate absence in returns.
...
18. Don't use Optional fields everywhere.
...
3. 1) Say out loud what Optional is for in one sentence.
