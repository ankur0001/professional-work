...
1. In the last episode, we worked through Reference Types.
2. If that made sense, today builds on it. If it was fuzzy, today usually makes it click.
3. Flags are experiments — change one variable, measure against SLOs.
...
2. Picture JVM Flags and Tuning clearly.
3. Here are the points that matter when code meets production:
4. Point 1: Set heap intentionally.
5. If you remember only one thing, make it that.
6. Point 2: Enable GC logging.
7. This is usually where tutorials stop — we will not.
8. Point 3: Prefer evidence over folklore.
9. Point 4: Document why a flag exists.
10. Point 5: Remove flags that no longer earn their keep.
...
5. Look at `java -Xms512m -Xmx512m -XX:+UseG1GC -Xlog:gc*:file=gc.log -jar app.jar`.
...
3. With JVM Flags and Tuning, mastery is not more jargon.
...
2. Question: First tuning step?
3. Answer: Metrics + GC logs under realistic load.
...
10. Prefer evidence over folklore.
...
18. Remove flags that no longer earn their keep.
...
3. 1) Say out loud what JVM Flags and Tuning is for in one sentence.
...
1. Landing the plane.
2. Today was JVM Flags and Tuning.
