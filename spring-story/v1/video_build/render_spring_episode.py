...
Output layout (common across episode PRs):
  ...
  spring-story/v1/Spring_Episode_XX_<Slug>_CAPTIONED.mp4
  spring-story/v1/Spring_Episode_XX.srt
  spring-story/v1/SOURCE.md

Usage:
  python3 spring-story/v1/video_build/render_v2_from_narrative.py --ep 1
  python3 spring-story/v1/video_build/render_v2_from_narrative.py --ep 1-5 --reuse-audio
  python3 spring-story/v1/video_build/render_v2_from_narrative.py --all
"""
...
ROOT = Path("/workspace")
EPISODES = ROOT / "spring-story" / "v1" / "episodes"
BUILD = ROOT / "spring-story" / "v1" / "video_build"
WORK = BUILD / "work"
OUT = EPISODES  # per-episode folders receive mp4/srt
SYS_PATH_TTS = BUILD
...
def find_existing_mp4(ep: int) -> Path | None:
    matches = sorted(OUT.glob(f"Spring_Episode_{ep:02d}_*_CAPTIONED.mp4"))
    matches = [p for p in matches if "PREVIEW" not in p.name]
    if matches:
        return matches[0]
    matches = sorted(OUT.glob(f"Spring_Episode_{ep:02d}_*.mp4"))
...
def update_episode_readme(folder: Path, clean_name: str, cap_name: str, srt_name: str) -> None:
    readme = folder / "README.md"
    if not readme.exists():
        return
    text = readme.read_text()
    if clean_name in text and "Video (clean)" in text:
        text = text.replace(
        ...
        return
    row = (
        f"| Video (clean) | [`{clean_name}`]({clean_name}) |\n"
        f"| Video (captioned) | [`{cap_name}`]({cap_name}) |\n"
        f"| Captions (SRT) | [`{srt_name}`]({srt_name}) |\n"
    )
    if "| Thumbnail |" in text:
        text = text.replace("| Thumbnail |", row + "| Thumbnail |", 1)
    text = text.replace(
        "> Video (`.mp4`) and captions (`.srt`) are produced in a later video-build pass. Upload metadata and thumbnail are ready now.",
        "> Video + SRT are ready in this folder (clean + captioned).",
    )
...
def render_one(path: Path, reuse_audio: bool = False, max_beats: int | None = None) -> dict:
    ...
    ep_out = path.parent
    srt = ep_out / f"Spring_Episode_{ep:02d}.srt"
    ...
    base = f"Spring_Episode_{ep:02d}_{slug}"
    if max_beats is not None:
        base = f"{base}_PREVIEW"
    mp4 = ep_out / f"{base}.mp4"
    cap = ep_out / f"{base}_CAPTIONED.mp4"
    ...
    src = ep_out / f"Spring_Episode_{ep:02d}_SOURCE.md"
    src.write_text(
...
def episode_files(spec: str) -> list[Path]:
    """Return narration.md paths for requested episode numbers."""
    by_n: dict[int, Path] = {}
    for folder in sorted(EPISODES.glob("ep*")):
        m = re.match(r"ep(\d+)", folder.name)
        if not m:
            continue
        narr = folder / "narration.md"
        if narr.exists():
            by_n[int(m.group(1))] = narr
    if spec == "all":
    ...
    for part in spec.split(","):
        ...
        if "-" in part:
            ...
            for n in range(int(a), int(b) + 1):
                if n not in by_n:
                    raise SystemExit(f"Episode {n} not found")
                out.append(by_n[n])
        else:
            n = int(part)
            if n not in by_n:
...
def main() -> None:
    ...
    (BUILD / "render_manifest.json").write_text(json.dumps(results, indent=2))
    print("DONE", len(results), "episodes")
