...
Hitting the database for every identical read is expensive. Caching is deliberate memory of recent answers.
...
So the natural question becomes: what does Spring give us so we do not keep paying that cost? The idea we need next is Caching.

At a practical level, Caching is the Spring mechanism you reach for when this pain shows up in a real codebase. Treat it as a tool with a clear job — not as a checklist item.
