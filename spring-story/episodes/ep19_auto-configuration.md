...
Boot’s central bet: if the classpath looks like a web app with a database, configure the obvious pieces for you.
...
So the natural question becomes: what does Spring give us so we do not keep paying that cost? The idea we need next is Auto Configuration.

At a practical level, Auto Configuration is the Spring mechanism you reach for when this pain shows up in a real codebase. Treat it as a tool with a clear job — not as a checklist item.
