...
An entity is not just a row mapped to fields. Lifecycle states control what SQL gets written.
...
So the natural question becomes: what does Spring give us so we do not keep paying that cost? The idea we need next is Entity Lifecycle.

At a practical level, Entity Lifecycle is the Spring mechanism you reach for when this pain shows up in a real codebase. Treat it as a tool with a clear job — not as a checklist item.
