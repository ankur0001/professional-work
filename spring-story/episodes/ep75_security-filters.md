...
Spring Security is a filter chain. Understanding that chain turns configuration into engineering.
...
So the natural question becomes: what does Spring give us so we do not keep paying that cost? The idea we need next is Security Filters.

At a practical level, Security Filters is the Spring mechanism you reach for when this pain shows up in a real codebase. Treat it as a tool with a clear job — not as a checklist item.
