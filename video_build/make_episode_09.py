...
SCENES = [
    ("hook", "hook", [
        "String is immutable text — central to APIs, logs, JSON, and HTTP.",
        "Every integration, log line, and payload depends on strings behaving predictably.",
        "Get strings right and comparisons, encoding, and memory stay under control.",
        "Get them wrong and subtle bugs ship to production.",
    ]),
    ("title", "title", [
        "Episode Nine.",
    ...
    ("immutable", "immutable", [
        "Once created, a String's content never changes.",
        "Concatenation does not edit the original — it creates a new String.",
        "Replace, trim, toUpperCase — each returns a fresh object.",
        "The old String stays in memory until garbage collection.",
        "That design is intentional — immutability enables sharing and thread safety.",
    ]),
    ("equality", "equality", [
        "Never use double equals for string content — that compares references.",
        "Use dot equals for value comparison.",
        "Better yet — quote PAID dot equals status — null-safe on the left.",
        "Two literals with the same text may share the pool — double equals can lie.",
        "In production, always compare meaning, not memory addresses.",
    ]),
    ("build", "build", [
        "Hot loops with plus build many temporary strings.",
        "Each plus in a loop can allocate a new object — expensive garbage.",
        "StringBuilder mutates in place — far less allocation pressure.",
        "Use StringBuilder for repeated concatenation in loops.",
        "For simple one-liners, plus is fine — the compiler often optimizes.",
    ]),
    ("charset", "charset", [
        "Bytes are not text until you pick a charset.",
        "UTF-8 is the default for modern APIs, JSON, and the web.",
        "Decoding without charset assumes platform defaults — fragile.",
        "toLowerCase without Locale breaks under Turkish locale rules.",
        "Always specify Charset or StandardCharsets dot UTF_8 explicitly.",
    ]),
    ("mistakes", "mistakes", [
        "Four common mistakes.",
        "One — double equals for text comparison instead of dot equals.",
        "Two — logging passwords or tokens in plain text.",
        "Three — unbounded user input without length or content checks.",
        "Four — raw strings scattered deep in domain logic instead of typed values.",
    ]),
    ("interview", "interview", [
        "Interview question — why is String immutable in Java?",
        "Sharing and the string pool — safe interning with fewer copies.",
        "Thread safety without locks on read-only data.",
        "Use StringBuilder when you mutate often in loops or buffers.",
    ]),
    ("teaser", "teaser", [
        "Text is structured. Next we structure behavior around data.",
