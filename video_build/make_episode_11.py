#!/usr/bin/env python3
"""Episode 11 — Access Modifiers. Narration + visuals authored together."""
...
AUDIO, FRAMES, CLIPS = ROOT / "audio_ep11", ROOT / "frames_ep11", ROOT / "clips_ep11"
VOICE = os.environ.get("KOKORO_VOICE", "am_michael")
SPEED = float(os.environ.get("KOKORO_SPEED", "0.97"))

SCENES = [
    ("hook", "hook", [
        "Objects need boundaries. Access modifiers draw those lines.",
        "Who can see this field? Who can call this method?",
        "Visibility is ownership encoded in syntax.",
        "Today we map private, package-private, protected, and public.",
        "Visibility is a design decision — not an afterthought.",
    ]),
    ("title", "title", [
        "Episode Eleven.",
    ...
    ("private", "private", [
        ...
        "Fields start private unless you have a reason.",
        "Helpers that are not part of the API stay private too.",
        "If everything is public, you have no boundary — only hope.",
        "Hope is not an architecture.",
        "Start private. Widen only with intent.",
    ]),
    ("package", "package", [
        "Package-private is underrated.",
        "Same package collaboration without publishing an API.",
    ...
    ("protected_public", "protected_public", [
        ...
        "Subclasses can reach it — so can the same package.",
        "public is the stable contract. Every public method is a promise to maintain.",
        "In libraries, public surface area is a long-term cost.",
        "Architect tip — treat public like a published product.",
        "Every public method is a maintenance promise.",
    ]),
    ("mistakes", "mistakes", [
        "Three common mistakes.",
        "One — public fields for convenience. Encapsulation dies quietly.",
    ...
    ("teaser", "teaser", [
        ...
        "Namespaces, boundaries, and ownership on disk.",
        "See you there.",
    ]),
]
...
def render_hook(progress, t):
    ...
    for i,(lab,col) in enumerate([("private",ORANGE),("package",BLUE),("protected",GREEN),("public",MUTED)]):
        ...
        d.text((x+60,520), lab, font=font(FONT_MONO_B,28), fill=mix(BG,col,a))
...
def render_title(progress, t):
    ...
    for txt,fnt,y,col in [
        ("EPISODE 11", font(FONT_BOLD,28), H//2-140, mix(BG,MUTED,a)),
        ("Access Modifiers", font(FONT_SERIF,64), H//2-30, mix(BG,WHITE,a)),
        ("ownership encoded as visibility", font(FONT_REG,30), H//2+70, mix(BG,MUTED,a)),
    ]:
        bbox=d.textbbox((0,0),txt,font=fnt); d.text(((W-(bbox[2]-bbox[0]))//2,y),txt,font=fnt,fill=col)
    return img.convert("RGB")

def render_levels(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "Narrow → Wide", font=font(FONT_SERIF,48), fill=WHITE)
    rows=[("private","this class only",ORANGE),("(default)","same package",BLUE),("protected","package + subclasses",GREEN),("public","everyone — a promise",MUTED)]
    for i,(name,note,col) in enumerate(rows):
        ...
        d.text((780,y+50), note, font=font(FONT_REG,30), fill=mix(BG,WHITE,a))
    return img.convert("RGB")

def render_private(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "Start Private", font=font(FONT_SERIF,48), fill=WHITE)
    a=ease_out_cubic(clamp(progress/0.4))
    d.rounded_rectangle([250,220,1670,780], radius=18, fill=mix(BG,SURFACE,a), outline=mix(BG,ORANGE,a), width=4)
    d.text((340,320), "private long amount;", font=font(FONT_MONO,36), fill=mix(BG,ORANGE,a))
    d.text((340,440), "private void validate() { … }", font=font(FONT_MONO,36), fill=mix(BG,BLUE,a))
    tip=ease_out_cubic(clamp((progress-0.5)/0.35))
    if tip>0:
        d.text((340,600), "If it is not an API — do not publish it.", font=font(FONT_REG,32), fill=mix(BG,WHITE,tip))
    return img.convert("RGB")

def render_package(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "Package-Private", font=font(FONT_SERIF,48), fill=WHITE)
    a=ease_out_cubic(clamp(progress/0.45))
    d.rounded_rectangle([200,200,1720,820], radius=18, fill=mix(BG,SURFACE,a), outline=mix(BG,BLUE,a), width=3)
    d.text((300,300), "class OrderHelper {  // no modifier", font=font(FONT_MONO,32), fill=mix(BG,WHITE,a))
    d.text((300,420), "    static Money sum(List<Order> xs) { … }", font=font(FONT_MONO,30), fill=mix(BG,GREEN,a))
    d.text((300,540), "}", font=font(FONT_MONO,32), fill=mix(BG,WHITE,a))
    tip=ease_out_cubic(clamp((progress-0.55)/0.3))
    if tip>0:
        d.text((300,680), "Same package only — collaboration without a public API.", font=font(FONT_REG,28), fill=mix(BG,MUTED,tip))
    return img.convert("RGB")

def render_protected_public(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160,60), "Protected & Public", font=font(FONT_SERIF,46), fill=WHITE)
    left=ease_out_cubic(clamp(progress/0.4)); right=ease_out_cubic(clamp((progress-0.35)/0.4))
    if left>0:
        d.rounded_rectangle([140,200,900,860], radius=18, fill=mix(BG,SURFACE,left), outline=mix(BG,GREEN,left), width=4)
        d.text((220,280), "protected", font=font(FONT_BOLD,40), fill=mix(BG,GREEN,left))
        for i,line in enumerate(["package + subclasses","extension hook","not a free public"]):
            d.text((220,420+i*90), f"•  {line}", font=font(FONT_REG,28), fill=mix(BG,WHITE,left))
    if right>0:
        d.rounded_rectangle([1020,200,1780,860], radius=18, fill=mix(BG,SURFACE,right), outline=mix(BG,ORANGE,right), width=4)
        d.text((1100,280), "public", font=font(FONT_BOLD,40), fill=mix(BG,ORANGE,right))
        for i,line in enumerate(["stable contract","everyone can call","long-term maintenance cost"]):
...
def render_mistakes(progress, t):
    ...
    items=[("01","Public fields for convenience","Keep fields private"),("02","Every helper marked public","Hide non-API helpers"),("03","protected as lazy public","Use protected intentionally")]
...
def render_interview(progress, t):
    ...
    q="Default access vs private?"
    bbox=d.textbbox((0,0),q,font=font(FONT_BOLD,34)); d.text(((W-(bbox[2]-bbox[0]))//2,190),q,font=font(FONT_BOLD,34),fill=WHITE)
    answers=[("private","only declaring class",ORANGE),("default","same package (package-private)",BLUE),("rule","narrowest visibility that works",GREEN)]
...
def render_teaser(progress, t):
    ...
    title="Packages"; bbox=d.textbbox((0,0),title,font=font(FONT_SERIF,72))
    d.text(((W-(bbox[2]-bbox[0]))//2, H//2-40), title, font=font(FONT_SERIF,72), fill=WHITE)
    sub="namespaces · boundaries · ownership"; bbox=d.textbbox((0,0),sub,font=font(FONT_REG,30))
    d.text(((W-(bbox[2]-bbox[0]))//2, H//2+60), sub, font=font(FONT_REG,30), fill=BLUE)
    d.text((W//2-90, H//2+150), "Episode 12", font=font(FONT_BOLD,30), fill=ORANGE)
    return img.convert("RGB")


RENDERERS = {"hook": render_hook, "title": render_title, "levels": render_levels, "private": render_private, "package": render_package, "protected_public": render_protected_public, "mistakes": render_mistakes, "interview": render_interview, "teaser": render_teaser}
...
def main():
    ...
    (ROOT / "ep11_durations.json").write_text(json.dumps(durations, indent=2))
    outs = [render_scene_clip(sid, r, durations[sid]) for sid, r, _ in SCENES]
    lst = ROOT / "concat_ep11.txt"
    with open(lst, "w") as f:
        for p in outs: f.write(f"file '{p}'\n")
    narrated = OUTPUT / "java_ep11_narrated.mp4"
    ...
    if abs(pace - 1.0) > 0.015:
        ...
        paced = OUTPUT / "java_ep11_paced.mp4"
        ...
        base = paced
    final = OUTPUT / "Java_Episode_11_Access_Modifiers.mp4"
    ...
    srt = OUTPUT / "Java_Episode_11.srt"; write_srt(durations, srt); shutil.copy2(srt, ARTIFACTS / srt.name)
    burned = OUTPUT / "Java_Episode_11_Access_Modifiers_CAPTIONED.mp4"
    ...
    vdir = ARTIFACTS / "ep11_verify"; vdir.mkdir(exist_ok=True)
    ...
    print(f"DONE Episode 11: {final_dur/60:.2f} min")
