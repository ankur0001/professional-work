#!/usr/bin/env python3
"""Episode 80 — Architecture Interview Wrap. Narration + visuals authored together."""
...
AUDIO, FRAMES, CLIPS = ROOT / "audio_ep80", ROOT / "frames_ep80", ROOT / "clips_ep80"
VOICE = os.environ.get("KOKORO_VOICE", "am_michael")
SPEED = float(os.environ.get("KOKORO_SPEED", "0.97"))

SCENES = [
    ("hook", "hook", [
        'Episode Seventy-Nine covered observability and resilience in distributed systems.',
        'You have traveled from Java syntax through JVM internals, patterns, and Spring.',
        'Architecture interviews reward structured thinking more than buzzword density.',
        'The best answers connect requirements to trade-offs and verification.',
        'Today — a reusable system-design framework using this series as your toolkit.',
        'This is Episode Eighty — the finale of The Java Story.',
    ]),
    ("title", "title", [
        'Episode Eighty.',
        'Architecture Interview Wrap.',
    ]),
    ("framework", "framework", [
        'A reusable architecture interview framework.',
        'Clarify functional requirements and non-functionals — latency, consistency, scale.',
        'Sketch a modular monolith first — extract services only with a reason.',
        'Define APIs, data ownership, and failure modes explicitly.',
        'Call out security, observability, and test strategy — seniors always do.',
        'Close with how you would measure success and evolve the design.',
    ]),
    ("java_toolkit", "java_toolkit", [
        'Map the series to interview moments.',
        'Language and collections — correct, idiomatic building blocks.',
        'Concurrency and JVM — explain latency, GC, and threading under load.',
        'Patterns — name structures when they clarify, not decorate.',
        'Spring — IoC, Boot, MVC, Data, Security as the delivery platform.',
        'Microservices and resilience — when distribution earns its cost.',
    ]),
    ("tradeoffs", "tradeoffs", [
        'Trade-offs interviewers listen for.',
        'Consistency versus availability — say which the product needs.',
        'Sync versus async — latency versus coupling versus complexity.',
        'Normalization versus read models — write simplicity versus query speed.',
        'Build versus buy — managed Kafka or a simpler queue.',
        'State the option you reject and why — that signals judgment.',
    ]),
    ("storytelling", "storytelling", [
        'How to narrate without drowning the room.',
        'Lead with the user journey — then zoom into the hottest path.',
        'Draw boxes sparingly — label ownership and data stores.',
        'Timebox deep dives — offer to go deeper on one component.',
        'When stuck — restate constraints and propose a boring working design.',
        'Boring and operable beats clever and fragile.',
    ]),
    ("checklist", "checklist", [
        'Final checklist before you say done.',
        'Requirements restated — scale numbers roughly estimated.',
        'API shapes and primary data models named.',
        'Authn, authz, and secret handling mentioned.',
        'Observability and failure injection path mentioned.',
        'Evolution path — what you would split or cache next.',
    ]),
    ("mistakes", "mistakes", [
        'Three common interview mistakes.',
        'One — jumping to Kafka and Kubernetes before requirements are clear.',
        'Two — ignoring data ownership and consistency until the last minute.',
        'Three — never mentioning how you would test or operate the design.',
        'Also — defending a choice you cannot explain under cross-examination.',
        'Judgment under constraints beats encyclopedic tooling lists.',
    ]),
    ("interview", "interview", [
        'Capstone prompt — design an order service for an online store.',
        'Clarify traffic, consistency for payments, and inventory constraints.',
        'Propose modular services or modules — Order, Payment, Inventory.',
        'Sync reserve inventory — async notify shipping — outbox for events.',
        'Secure APIs, trace requests, break circuit on payment timeouts.',
        'Ship a boring design first — then scale the measured hotspots.',
    ]),
    ("teaser", "teaser", [
        'That is the end of The Java Story — eighty episodes.',
        'From hello world to architecture interviews — one continuous arc.',
        'Rebuild any episode from its make_episode script when you need a refresher.',
...
def render_hook(progress, t):
    ...
    labs = [('requirements', 'ORANGE'), ('trade-offs', 'BLUE'), ('verify', 'GREEN')]
...
def render_title(progress, t):
    ...
    for txt, fnt, y, col in [
        ('EPISODE 80', font(FONT_BOLD, 28), H // 2 - 140, mix(BG, MUTED, a)),
        ('Architecture Interview Wrap', font(FONT_SERIF, 46), H // 2 - 30, mix(BG, WHITE, a)),
        ('series finale · system design framework', font(FONT_REG, 26), H // 2 + 70, mix(BG, MUTED, a)),
    ]:
        bbox = d.textbbox((0, 0), txt, font=fnt); d.text(((W - (bbox[2] - bbox[0])) // 2, y), txt, font=fnt, fill=col)
    return img.convert("RGB")

def render_framework(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Answer Framework', font=font(FONT_SERIF, 44), fill=WHITE)
    steps = [('clarify NFRs', 'ORANGE'), ('boring design', 'BLUE'), ('data + APIs', 'GREEN'), ('secure + observe', 'RED')]
...
def render_java_toolkit(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Series Toolkit', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('language + JVM', 'correctness under load', 'ORANGE'), ('patterns + Spring', 'structure + delivery', 'BLUE'), ('distributed ops', 'split only when earned', 'GREEN')]
...
def render_tradeoffs(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Trade-offs to Voice', font=font(FONT_SERIF, 40), fill=WHITE)
    ...
    lines = ['consistency vs availability', 'sync vs async coupling', 'normalize vs read models', 'build vs managed buy', 'reject an option with reasons']
    for i, line in enumerate(lines):
        aa = ease_out_cubic(clamp((progress - i * 0.08) / 0.22))
        d.text((260, 280 + i * 100), line, font=font(FONT_MONO, 26), fill=mix(BG, WHITE, aa))
    return img.convert("RGB")

def render_storytelling(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Narration Tips', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('user journey first', 'then hottest path', 'ORANGE'), ('timebox depth', 'offer one deep dive', 'BLUE'), ('boring operable', 'beats clever fragile', 'GREEN')]
    for i, (k, v, col) in enumerate(items):
        a = ease_out_cubic(clamp((progress - i * 0.18) / 0.3))
        if a <= 0: continue
        y = 220 + i * 200
...
def render_checklist(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), 'Final Checklist', font=font(FONT_SERIF, 44), fill=WHITE)
    items = [('reqs + scale math', 'rough, honest numbers', 'ORANGE'), ('auth + observe', 'seniors always mention', 'BLUE'), ('evolution path', "what you'd change next", 'GREEN')]
    for i, (k, v, col) in enumerate(items):
        a = ease_out_cubic(clamp((progress - i * 0.18) / 0.3))
        if a <= 0: continue
        y = 220 + i * 200
...
def render_mistakes(progress, t):
    ...
    items = [('01', 'tools before requirements', 'clarify constraints first'), ('02', 'skip data ownership', 'name stores + consistency'), ('03', 'no ops story', 'test + observe + on-call')]
...
def render_interview(progress, t):
    ...
    q = 'Design an order service'
    bbox = d.textbbox((0, 0), q, font=font(FONT_BOLD, 30)); d.text(((W - (bbox[2] - bbox[0])) // 2, 190), q, font=font(FONT_BOLD, 30), fill=WHITE)
    answers = [('clarify', 'traffic · payments · inventory', 'ORANGE'), ('modules', 'Order · Payment · Inventory', 'BLUE'), ('harden', 'secure · trace · breakers', 'GREEN')]
...
def render_teaser(progress, t):
    ...
    title = 'Series Complete'; bbox = d.textbbox((0, 0), title, font=font(FONT_SERIF, 42))
    d.text(((W - (bbox[2] - bbox[0])) // 2, H // 2 - 40), title, font=font(FONT_SERIF, 42), fill=WHITE)
    sub = 'eighty episodes · go build'; bbox = d.textbbox((0, 0), sub, font=font(FONT_REG, 28))
...
RENDERERS = {
    ...
    "java_toolkit": render_java_toolkit,
    "tradeoffs": render_tradeoffs,
    "storytelling": render_storytelling,
...
def main():
    ...
    (ROOT / "ep80_durations.json").write_text(json.dumps(durations, indent=2))
    outs = [render_scene_clip(sid, r, durations[sid]) for sid, r, _ in SCENES]
    lst = ROOT / "concat_ep80.txt"
    with open(lst, "w") as f:
        for p in outs: f.write(f"file '{p}'\n")
    narrated = OUTPUT / "java_ep80_narrated.mp4"
    ...
    if abs(pace - 1.0) > 0.015:
        ...
        paced = OUTPUT / "java_ep80_paced.mp4"
        ...
        base = paced
    final = OUTPUT / "Java_Episode_80_Architecture_Interview_Wrap.mp4"
    ...
    srt = OUTPUT / "Java_Episode_80.srt"; write_srt(durations, srt); shutil.copy2(srt, ARTIFACTS / srt.name)
    burned = OUTPUT / "Java_Episode_80_Architecture_Interview_Wrap_CAPTIONED.mp4"
    ...
    vdir = ARTIFACTS / "ep80_verify"; vdir.mkdir(exist_ok=True)
    for ts, name in [('00:00:12', '01_hook'), ('00:00:50', '02_framework'), ('00:01:40', '03_toolkit'), ('00:02:30', '04_tradeoffs'), ('00:03:20', '05_capstone')]:
        subprocess.run(["ffmpeg", "-y", "-ss", ts, "-i", str(final), "-frames:v", "1", str(vdir / f"{name}.jpg")], capture_output=True)
    final_dur = probe(final)
    print(f"DONE Episode 80: {final_dur/60:.2f} min")
