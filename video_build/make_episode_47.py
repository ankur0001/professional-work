#!/usr/bin/env python3
"""Episode 47 — ForkJoinPool & Work-Stealing. Narration + visuals authored together."""
...
AUDIO, FRAMES, CLIPS = ROOT / "audio_ep47", ROOT / "frames_ep47", ROOT / "clips_ep47"
VOICE = os.environ.get("KOKORO_VOICE", "am_michael")
SPEED = float(os.environ.get("KOKORO_SPEED", "0.97"))

SCENES = [
    ("hook", "hook", [
        'Episode Forty-Six showed CompletableFuture on the common pool.',
        'That pool is a ForkJoinPool — built for divide-and-conquer parallelism.',
        'Workers split big tasks into smaller pieces and join results.',
        'Idle threads steal work from busy neighbors — work-stealing.',
        'RecursiveTask and RecursiveAction model fork-join decomposition.',
        'Today — ForkJoinPool, work-stealing, and when recursive parallelism pays off.',
    ]),
    ("title", "title", [
        'Episode Forty-Seven.',
        'ForkJoinPool and Work-Stealing.',
    ]),
    ("fork_join_basics", "fork_join_basics", [
        'ForkJoinPool is an ExecutorService tuned for parallel decomposition.',
        'fork splits a task into subtasks — join waits for subtask results.',
        'invoke runs a single task and blocks until it completes.',
        'submit returns a ForkJoinTask — use when you need a handle.',
        'Pool size defaults to available processors — tune for your workload.',
        'Think tree-shaped computation — split down, merge up.',
    ]),
    ("work_stealing", "work_stealing", [
        'Each worker maintains a deque of tasks — LIFO for its own work.',
        'When a worker runs dry, it steals from another worker deque — FIFO end.',
        'Stealing balances load without a central coordinator bottleneck.',
        'Fine-grained tasks keep workers busy — coarse tasks leave threads idle.',
        'Work-stealing shines when task sizes are uneven or unpredictable.',
        'Too many tiny tasks add overhead — batch until splits are worthwhile.',
    ]),
    ("recursive_task", "recursive_task", [
        'RecursiveTask of V extends ForkJoinTask — compute returns a value.',
        'Override compute — fork children, join them, combine results.',
        'Example — parallel sum of a large array by halving ranges.',
        'fork enqueues a subtask on the current pool.',
        'join blocks the current worker until the child completes.',
        'Use RecursiveTask when the result is a computed value.',
    ]),
    ("recursive_action", "recursive_action", [
        'RecursiveAction extends ForkJoinTask of Void — side effects only.',
        'Override compute — fork subtasks, join, no return value.',
        'Example — parallel forEach over a tree or matrix in place.',
        'Same fork-join pattern — split work, wait for children.',
        'Choose RecursiveAction when you mutate shared structures carefully.',
        'Or when each leaf performs independent I/O or logging.',
    ]),
    ("when_to_use", "when_to_use", [
        'When ForkJoinPool fits.',
        'CPU-bound divide-and-conquer — mergesort, matrix multiply, tree walks.',
        'Parallel streams use the common pool under the hood.',
        'Recursive decomposition with cheap merge steps.',
        'When not — many blocking I/O tasks — use a cached thread pool.',
        'When not — tiny uniform tasks — overhead beats parallelism.',
    ]),
    ("mistakes", "mistakes", [
        'Three common mistakes.',
        'One — blocking inside compute — pins a worker and kills stealing.',
        'Two — shared mutable state without coordination — data races.',
        'Three — forking too deep — millions of tasks overwhelm the pool.',
        'Also — using commonPool for blocking work — starves parallel streams.',
        'Size tasks so fork overhead stays small relative to real work.',
    ]),
    ("interview", "interview", [
        'Interview question — how does work-stealing work?',
        'Each thread has a deque — processes own tasks LIFO.',
        'Idle threads steal from the opposite end — FIFO — of another deque.',
        'Balances load without a global queue lock on every operation.',
        'RecursiveTask returns a value — RecursiveAction is void side effects.',
        'Mention parallel streams and CompletableFuture common pool.',
    ]),
    ("teaser", "teaser", [
        'Pools share threads across tasks. What about per-thread state?',
        'Episode Forty-Eight — ThreadLocal.',
        'Per-thread variables, inheritance, and leak hazards.',
        'See you there.',
    ]),
]
...
def render_hook(progress, t):
    ...
    for i, (lab, col) in enumerate([("fork", ORANGE), ("steal", BLUE), ("join", GREEN)]):
        ...
        d.text((x + 100, 520), lab, font=font(FONT_BOLD, 28), fill=mix(BG, col, a))
...
def render_title(progress, t):
    ...
    for txt, fnt, y, col in [
        ("EPISODE 47", font(FONT_BOLD, 28), H // 2 - 140, mix(BG, MUTED, a)),
        ("ForkJoinPool", font(FONT_SERIF, 56), H // 2 - 30, mix(BG, WHITE, a)),
        ("work-stealing · RecursiveTask", font(FONT_REG, 28), H // 2 + 70, mix(BG, MUTED, a)),
    ]:
        bbox = d.textbbox((0, 0), txt, font=fnt); d.text(((W - (bbox[2] - bbox[0])) // 2, y), txt, font=fnt, fill=col)
    return img.convert("RGB")


def render_fork_join_basics(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), "ForkJoinPool", font=font(FONT_SERIF, 48), fill=WHITE)
    items = [("fork()", "split into subtasks", ORANGE), ("join()", "wait for child result", BLUE), ("invoke()", "run and block for result", GREEN)]
    for i, (k, v, col) in enumerate(items):
        ...
        d.text((580, y + 50), v, font=font(FONT_REG, 28), fill=mix(BG, WHITE, a))
    return img.convert("RGB")


def render_work_stealing(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), "Work-Stealing", font=font(FONT_SERIF, 46), fill=WHITE)
    ...
    lines = [
        "Worker A deque: [task3, task2, task1]  ← LIFO own work",
        "Worker B idle → steals from A tail (FIFO)",
        "Balances load without central bottleneck",
    ]
    for i, line in enumerate(lines):
        aa = ease_out_cubic(clamp((progress - i * 0.12) / 0.28))
        d.text((280, 320 + i * 140), line, font=font(FONT_MONO, 26), fill=mix(BG, WHITE, aa))
    return img.convert("RGB")


def render_recursive_task(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), "RecursiveTask<V>", font=font(FONT_SERIF, 44), fill=WHITE)
    a = ease_out_cubic(clamp(progress / 0.4))
    ...
    for i, line in enumerate(lines):
        ...
        d.text((260, 280 + i * 110), line, font=font(FONT_MONO, 24), fill=mix(BG, WHITE, aa))
    return img.convert("RGB")


def render_recursive_action(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), "RecursiveAction", font=font(FONT_SERIF, 44), fill=WHITE)
    a = ease_out_cubic(clamp(progress / 0.4))
...
def render_when_to_use(progress, t):
    ...
    good = [("divide-and-conquer CPU work", GREEN), ("parallel streams / common pool", ORANGE), ("cheap merge, sizable chunks", BLUE)]
    ...
    if a > 0:
        ...
        d.text((280, 760), "✗  blocking I/O inside compute()", font=font(FONT_BOLD, 28), fill=mix(BG, RED, a))
    return img.convert("RGB")
...
def render_mistakes(progress, t):
    ...
    items = [("01", "blocking in compute", "pins worker threads"), ("02", "mutable shared state", "race conditions"), ("03", "over-forking", "task storm overhead")]
...
def render_interview(progress, t):
    ...
    answers = [("deque per worker", "LIFO for own tasks", ORANGE), ("idle steals", "FIFO from neighbor tail", BLUE), ("RecursiveTask vs Action", "value vs void", GREEN)]
...
def render_teaser(progress, t):
    ...
    title = "ThreadLocal"; bbox = d.textbbox((0, 0), title, font=font(FONT_SERIF, 50))
    d.text(((W - (bbox[2] - bbox[0])) // 2, H // 2 - 40), title, font=font(FONT_SERIF, 50), fill=WHITE)
    sub = "per-thread state · inheritance"; bbox = d.textbbox((0, 0), sub, font=font(FONT_REG, 28))
    d.text(((W - (bbox[2] - bbox[0])) // 2, H // 2 + 60), sub, font=font(FONT_REG, 28), fill=BLUE)
    d.text((W // 2 - 90, H // 2 + 150), "Episode 48", font=font(FONT_BOLD, 30), fill=ORANGE)
    return img.convert("RGB")


RENDERERS = {
    "hook": render_hook, "title": render_title, "fork_join_basics": render_fork_join_basics,
    "work_stealing": render_work_stealing, "recursive_task": render_recursive_task,
    "recursive_action": render_recursive_action, "when_to_use": render_when_to_use,
...
def main():
    ...
    (ROOT / "ep47_durations.json").write_text(json.dumps(durations, indent=2))
    outs = [render_scene_clip(sid, r, durations[sid]) for sid, r, _ in SCENES]
    lst = ROOT / "concat_ep47.txt"
    with open(lst, "w") as f:
        for p in outs: f.write(f"file '{p}'\n")
    narrated = OUTPUT / "java_ep47_narrated.mp4"
    ...
    if abs(pace - 1.0) > 0.015:
        ...
        paced = OUTPUT / "java_ep47_paced.mp4"
        ...
        base = paced
    final = OUTPUT / "Java_Episode_47_ForkJoinPool.mp4"
    ...
    srt = OUTPUT / "Java_Episode_47.srt"; write_srt(durations, srt); shutil.copy2(srt, ARTIFACTS / srt.name)
    burned = OUTPUT / "Java_Episode_47_ForkJoinPool_CAPTIONED.mp4"
    ...
    vdir = ARTIFACTS / "ep47_verify"; vdir.mkdir(exist_ok=True)
    for ts, name in [('00:00:12', '01_hook'), ('00:00:50', '02_work_stealing'), ('00:01:40', '03_recursive_task'), ('00:02:30', '04_when_to_use'), ('00:03:20', '05_interview')]:
        subprocess.run(["ffmpeg", "-y", "-ss", ts, "-i", str(final), "-frames:v", "1", str(vdir / f"{name}.jpg")], capture_output=True)
    final_dur = probe(final)
    print(f"DONE Episode 47: {final_dur/60:.2f} min")
