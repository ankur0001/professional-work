#!/usr/bin/env python3
"""Episode 62 — JVM Flags & Tuning Basics. Narration + visuals authored together."""
...
AUDIO, FRAMES, CLIPS = ROOT / "audio_ep62", ROOT / "frames_ep62", ROOT / "clips_ep62"
VOICE = os.environ.get("KOKORO_VOICE", "am_michael")
SPEED = float(os.environ.get("KOKORO_SPEED", "0.97"))

SCENES = [
    ("hook", "hook", [
        'Episode Sixty-One covered soft, weak, and phantom references.',
        'Reference types shape object lifetime — JVM flags shape runtime behavior.',
        'Every production JVM starts with dozens of implicit defaults.',
        'Heap size, collector choice, and logging flags change latency and stability.',
        'Copying flags from a blog post without measurement is a common failure mode.',
        'Today — JVM flags, heap sizing, GC switches, and a measurement-first mindset.',
    ]),
    ("title", "title", [
        'Episode Sixty-Two.',
        'JVM Flags and Tuning Basics.',
    ]),
    ("heap_sizing", "heap_sizing", [
        'Heap sizing starts with -Xms and -Xmx.',
        '-Xms sets initial heap — -Xmx sets maximum heap the JVM may use.',
        'Matching Xms to Xmx avoids resize pauses during steady state.',
        'Too small — frequent GC and OutOfMemoryError under load.',
        'Too large — long GC pauses and wasted RAM on shared hosts.',
        'Start from observed usage under realistic traffic — add headroom, not guesses.',
    ]),
    ("gc_flags", "gc_flags", [
        'Collector flags select the garbage collector implementation.',
        '-XX:+UseG1GC — default general-purpose collector since Java 9.',
        '-XX:+UseZGC — low-pause collector for large heaps and strict latency.',
        '-XX:+UseParallelGC — throughput-oriented for batch workloads.',
        'MaxGCPauseMillis tunes G1 pause target — best effort, not a guarantee.',
        'Collector choice is empirical — validate with GC logs on your workload.',
    ]),
    ("diagnostic_flags", "diagnostic_flags", [
        'Diagnostic flags make invisible behavior visible.',
        '-Xlog:gc* enables unified GC logging in modern JDK releases.',
        '-XX:+HeapDumpOnOutOfMemoryError writes a heap dump on OOM.',
        '-XX:ErrorFile=path captures fatal JVM error details.',
        'PrintFlagsFinal lists every flag and its effective value at startup.',
        'Flight Recorder and async profilers complement flags — use them before tuning blind.',
    ]),
    ("measurement_mindset", "measurement_mindset", [
        'Tuning without measurement is guessing.',
        'Establish a baseline — latency, throughput, GC pause times, heap usage.',
        'Change one variable at a time — flag, heap size, or collector.',
        'Replay production traffic or run load tests that match real patterns.',
        'Compare before and after with the same dataset and duration.',
        'Document what you changed and why — future you will thank present you.',
    ]),
    ("common_flags", "common_flags", [
        'A practical starter flag set for services.',
        'java -Xms4g -Xmx4g -XX:+UseG1GC -XX:MaxGCPauseMillis=200.',
        'Add -Xlog:gc*:file=gc.log:time,uptime,level,tags for GC analysis.',
        'Container deployments — respect cgroup memory limits with -XX:MaxRAMPercentage.',
        'Never set flags you cannot explain in an incident postmortem.',
        'Flags are tools — the goal is reliable behavior under real load.',
    ]),
    ("mistakes", "mistakes", [
        'Three common mistakes.',
        'One — copying another team\'s flags without matching workload or heap.',
        'Two — setting Xmx to all available RAM — no room for metaspace or OS cache.',
        'Three — tuning GC before fixing allocation hotspots in application code.',
        'Also — changing five flags at once — impossible to attribute improvements.',
        'Measure first, tune second, verify third — always in that order.',
    ]),
    ("interview", "interview", [
        'Interview question — how do you tune JVM flags for a service?',
        'Start with baseline metrics — latency percentiles, GC logs, heap usage.',
        'Size heap from observed peak plus headroom — match Xms and Xmx when stable.',
        'Choose collector for workload — G1 default, ZGC for strict pause goals.',
        'Enable GC logging and OOM heap dumps before changing anything.',
        'Change one knob, re-test, document — never tune from blog posts alone.',
    ]),
    ("teaser", "teaser", [
        'Flags control the JVM — object layout controls how much memory each instance uses.',
        'Episode Sixty-Three — Object Layout and Compressed Oops.',
        'Headers, padding, and pointer compression on 64-bit heaps.',
        'See you there.',
    ]),
]
...
def render_hook(progress, t):
    ...
    for i, (lab, col) in enumerate([("-Xmx", ORANGE), ("GC", BLUE), ("-Xlog", GREEN)]):
        ...
        d.text((x + 140, 520), lab, font=font(FONT_MONO, 32), fill=mix(BG, col, a))
...
def render_title(progress, t):
    ...
    for txt, fnt, y, col in [
        ("EPISODE 62", font(FONT_BOLD, 28), H // 2 - 140, mix(BG, MUTED, a)),
        ("JVM Flags & Tuning", font(FONT_SERIF, 48), H // 2 - 30, mix(BG, WHITE, a)),
        ("-Xmx · GC flags · measure first", font(FONT_REG, 26), H // 2 + 70, mix(BG, MUTED, a)),
    ]:
        bbox = d.textbbox((0, 0), txt, font=fnt); d.text(((W - (bbox[2] - bbox[0])) // 2, y), txt, font=fnt, fill=col)
    return img.convert("RGB")


def render_heap_sizing(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), "Heap Sizing", font=font(FONT_SERIF, 44), fill=WHITE)
    items = [("-Xms", "initial heap", ORANGE), ("-Xmx", "maximum heap", BLUE), ("match both", "avoid resize pauses", GREEN)]
...
def render_gc_flags(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
...
def render_diagnostic_flags(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), "Diagnostic Flags", font=font(FONT_SERIF, 40), fill=WHITE)
    flags = [("-Xlog:gc*", "GC logging", ORANGE), ("HeapDumpOnOOM", "capture on failure", BLUE), ("PrintFlagsFinal", "see all values", GREEN)]
...
def render_measurement_mindset(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), "Measure First", font=font(FONT_SERIF, 44), fill=WHITE)
    steps = [("baseline", ORANGE), ("one change", BLUE), ("re-test", GREEN), ("document", RED)]
...
def render_common_flags(progress, t):
    img = base_canvas(t); d = ImageDraw.Draw(img)
    d.text((160, 60), "Starter Flag Set", font=font(FONT_SERIF, 40), fill=WHITE)
    a = ease_out_cubic(clamp(progress / 0.4))
...
def render_mistakes(progress, t):
    ...
    items = [("01", "copy blog flags", "match your workload"), ("02", "Xmx = all RAM", "leave OS headroom"), ("03", "tune before fix", "fix allocations first")]
...
def render_interview(progress, t):
    ...
    q = "How do you tune JVM flags?"
    bbox = d.textbbox((0, 0), q, font=font(FONT_BOLD, 36)); d.text(((W - (bbox[2] - bbox[0])) // 2, 190), q, font=font(FONT_BOLD, 36), fill=WHITE)
    answers = [("baseline", "metrics + GC logs", ORANGE), ("heap", "observed peak + headroom", BLUE), ("one knob", "measure, change, verify", GREEN)]
...
def render_teaser(progress, t):
    ...
    title = "Object Layout & Compressed Oops"; bbox = d.textbbox((0, 0), title, font=font(FONT_SERIF, 42))
    d.text(((W - (bbox[2] - bbox[0])) // 2, H // 2 - 40), title, font=font(FONT_SERIF, 42), fill=WHITE)
    sub = "headers · padding · UseCompressedOops"; bbox = d.textbbox((0, 0), sub, font=font(FONT_REG, 28))
    d.text(((W - (bbox[2] - bbox[0])) // 2, H // 2 + 60), sub, font=font(FONT_REG, 28), fill=BLUE)
    d.text((W // 2 - 90, H // 2 + 150), "Episode 63", font=font(FONT_BOLD, 30), fill=ORANGE)
    return img.convert("RGB")


RENDERERS = {
    "hook": render_hook, "title": render_title, "heap_sizing": render_heap_sizing,
    "gc_flags": render_gc_flags, "diagnostic_flags": render_diagnostic_flags,
    "measurement_mindset": render_measurement_mindset, "common_flags": render_common_flags,
...
def synth_scene_audio(pipeline, scene_id, beats):
    ...
    for i, beat in enumerate(beats):
        ...
        parts.append(wav)
        if i < len(beats) - 1:
            gap = 0.30 if any(k in text for k in ("Interview", "Three common", "A practical", "Tuning")) else (0.28 if text.endswith("?") else 0.12)
...
def main():
    ...
    (ROOT / "ep62_durations.json").write_text(json.dumps(durations, indent=2))
    outs = [render_scene_clip(sid, r, durations[sid]) for sid, r, _ in SCENES]
    lst = ROOT / "concat_ep62.txt"
    with open(lst, "w") as f:
        for p in outs: f.write(f"file '{p}'\n")
    narrated = OUTPUT / "java_ep62_narrated.mp4"
    ...
    if abs(pace - 1.0) > 0.015:
        ...
        paced = OUTPUT / "java_ep62_paced.mp4"
        ...
        base = paced
    final = OUTPUT / "Java_Episode_62_JVM_Flags_Tuning.mp4"
    ...
    srt = OUTPUT / "Java_Episode_62.srt"; write_srt(durations, srt); shutil.copy2(srt, ARTIFACTS / srt.name)
    burned = OUTPUT / "Java_Episode_62_JVM_Flags_Tuning_CAPTIONED.mp4"
    ...
    vdir = ARTIFACTS / "ep62_verify"; vdir.mkdir(exist_ok=True)
    for ts, name in [('00:00:12', '01_hook'), ('00:00:50', '02_heap'), ('00:01:40', '03_gc_flags'), ('00:02:30', '04_measure'), ('00:03:20', '05_interview')]:
        subprocess.run(["ffmpeg", "-y", "-ss", ts, "-i", str(final), "-frames:v", "1", str(vdir / f"{name}.jpg")], capture_output=True)
    final_dur = probe(final)
    print(f"DONE Episode 62: {final_dur/60:.2f} min")
